...
describe("SQLite platform bootstrap", () => {
  it("creates and migrates a missing first-start data root with private permissions", async () => {
    ...
    try {
      ...
      expect(readScalar(context.client, "SELECT COUNT(*) FROM __drizzle_migrations")).toBe(19);
  ...
  it("keeps repeated migration runs idempotent", async () => {
    ...
    try {
      expect(readScalar(second.client, "SELECT COUNT(*) FROM __drizzle_migrations")).toBe(19);
  ...
  it("upgrades existing profiles with empty projections without changing overrides", async () => {
    ...
    const migrationDirectories = (await readdir(defaultMigrationsFolder, { withFileTypes: true }))
      ...
      .sort();
    expect(migrationDirectories.at(-1)).toBe("20260814120000_add_taste_blueprint");
  ...
  it("upgrades the last production schema while preserving representative user data", async () => {
    ...
    try {
      ...
      expect(readScalar(upgraded.client, "SELECT COUNT(*) FROM __drizzle_migrations")).toBe(19);
      expect(
        upgraded.client
          .prepare(
            `
