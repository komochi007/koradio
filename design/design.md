...
* Radio：05 代表 04–06。仅 desktop standalone PWA 以 `960 × 1600px` 画布等比缩小适配实际可视区域，不因窄窗口或浏览器缩放切换到 Mobile 骨架；页面外层不得滚动或隐藏内容。普通浏览器和手机继续使用响应式布局。顶部品牌、底部场景输入和底部导航保持固定，队列与 DJ 对话等长内容只能在自身卡片内滚动。Tablet 与 Desktop 的 Radio rail 最大宽度保持 `816px`，宽屏不得增加侧栏或第二列。
