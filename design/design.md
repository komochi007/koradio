...
* Radio：05 代表 04–06。Desktop PWA 以 `960 × 1600px` 画布等比适配窗口，页面外层不得滚动；顶部品牌、底部场景输入和底部导航保持固定，队列与 DJ 对话等长内容只能在自身卡片内滚动。Tablet 与 Desktop 的 Radio rail 最大宽度保持 `816px`，宽屏不得增加侧栏或第二列。
...
* 一级页面底部使用固定胶囊导航，规格为 `620 × 88px`，距离画布底部 `32px`；选中态使用白色圆角矩形底，不得因断点变为圆形。
