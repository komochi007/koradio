...
- 标题与艺术家按同一左边线对齐，操作按钮按同一右边线对齐；文本溢出单行截断并提供完整可访问名称。当前歌曲使用克制的细均衡器、主文字和轻微表面差，不使用整行绿色填充。
- 生成中只显示当日生成状态，不展示旧列表；失败显示“今日推荐暂未生成”和重新生成；结束显示“今日推荐已播完”并保留返回原节目。

### 10.3 Queue 来源 Switch

- Queue 标题的 `TRACKS` 右侧放置约 `104 × 28px` 的双段胶囊，仅包含 `RADIO` 与 `DAILY` 文字。外框、内滑块、文字基线和标题栏垂直中心必须精确对齐。
- 滑块表示当前基础来源，使用 180ms ease-out-quart 横向 transform；禁止使用图标、状态点或大面积绿色。无目标来源时对应方向不可点击并降低对比度。
- 胶囊支持 Tab 聚焦、左右方向键和点击，使用 `aria-label` 与 `aria-pressed` 表达来源；临时插播期间仍显示基础来源。
