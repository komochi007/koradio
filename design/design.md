...
响应式代表页使用 `390 × 844px`、`834 × 1194px` 两组固定验收 viewport。精细指针桌面设备在窗口宽度达到 `960px` 时使用居中的 `960 × 1600px` 固定产品画布；窗口高度不足 `1600px` 时显示尺寸提示，不渲染裁切后的产品内容。`960 × 1600px` Prototype 继续使用 4.6、5.11 与 8.2 的固定坐标，不因响应式实现改变基线几何。
...
| Desktop `≥ 960 × 1600px` | 固定画布外侧居中承载 | `40px` | 固定 `620 × 88px`，画布底部 `32px` |

五类布局按以下规则由代表页继承：

* Profile：03 代表 02–03。顶部工具与表单内容列共享安全边缘；移动端可保留单列内部滚动。固定桌面画布中表单内容只能在受限内部区域滚动，页面本身不得滚动。
* Radio：05 代表 04–06。顶部品牌、底部场景输入和底部导航保持固定。固定桌面画布中时间、主状态、队列、DJ 状态与对话使用原型固定 slot，不产生连续页面滚动；Tablet 的 Radio rail 最大宽度保持 `816px`，宽屏不得增加侧栏或第二列。
...
* Management：10 代表 09–13。顶部工具、管理内容列与导航共享固定边缘，最大宽度保持 `840px`。固定桌面画布只允许列表、队列等内部内容区域滚动且隐藏滚动条；移动端将同级并排卡片纵向排列，保持信息顺序，不使用后台侧栏。
* Settings：14 代表 14–15。顶部工具、底部操作与导航固定，最大宽度保持 `832px`。固定桌面画布中的长表单和诊断内容只可在内部区域滚动且隐藏滚动条；移动端仍保留状态文字、Codex 配置、内置 Provider 能力状态、偏好和本地数据操作，不把表单改成多栏或隐藏次级状态。

所有布局族共同遵循：

* 固定桌面画布的产品外壳、页面和一级内容区不得滚动或显示滚动条；仅歌词、串讲、列表、诊断与表单等内部内容区域可滚动，且必须为固定顶部、底部操作与 safe area 预留空间，不得被导航遮挡。
...
* PWA 固定桌面模式按原尺寸居中呈现画布，不做自动缩放；尺寸不足时显示尺寸提示，不能裁切或回退为桌面响应式布局。
