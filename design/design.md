...
- 声场只使用黑白灰，通过少量主结构线、中层主体线、外围环境线和三层低透明曲面建立明暗层次；高亮只能是沿整体曲面的低对比渐变，不使用绿色节点、彩色渐变、霓虹或重发光。水平与垂直 mask 必须让左、右、上、下自然渐隐；右侧允许在完成渐隐后被容器裁切。Desktop 主体宽度约为 Header 的 `78%`，紧凑窗口约为 `94%`，通过可视范围变化而非横向压缩适配。
- 声场默认保持静态，不使用随机数、逐帧重算或 resize 驱动的形态切换，保证窗口变化时路径稳定且不闪烁。若未来加入动效，必须先证明其在 Reduce Motion、窗口缩放和长时间运行下不破坏确定性与克制感。
