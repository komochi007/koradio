...
* Management：10 代表 09–13。顶部工具、管理内容列与导航共享固定边缘；内容层独立滚动，最大宽度保持 `840px`。移动端将同级并排卡片纵向排列，保持信息顺序，不使用后台侧栏。
* Settings：14 代表 14–15。顶部工具、底部操作与导航固定，设置内容独立滚动，最大宽度保持 `832px`。移动端仍保留状态文字、Codex 配置、内置 Provider 能力状态、偏好和本地数据操作，不把表单改成多栏或隐藏次级状态。
