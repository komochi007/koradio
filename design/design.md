...
* Detail：08 代表 07–08。Sheet 始终通过窗口根节点覆盖整个 viewport，外层与白色节目面左右不得保留画布边距；声场与节目面保持上下结构，节目内容列在 Mobile 使用页面安全边距，在 Tablet 与 Desktop 最大为 `848px`。状态、关闭、歌曲进度、主内容卡、节目进度和单一播放控制均不得移除。
...
当前句或当前歌词行使用浅色面主文本 `#14161A`；已读内容使用 `#898A86`，在 `#F5F3F6` 大号正文卡片上保持 `3.15:1` 对比度；未播放内容使用中灰 `#767A80`。避免使用大面积绿色文字高亮，可使用非常轻的词级绿色柔光或小状态点辅助定位，不使用绿色侧边条。

若歌词或 DJ 串讲没有真实逐字时间戳，逐字进度按当前行的可读文字长度在该行时段内估算：中文按单个汉字推进，英文按单词推进，标点与空白跟随前一个单位。该估算只影响高亮进度，不改变 canonical timeline、播放位置或文本内容。

歌词态普通、已读、待播和静态歌词使用 `clamp(24px, 3.75vw, 36px)`、`1.55` 行高；当前歌词使用 `clamp(34px, 5vw, 48px)`、`1.35` 行高并保留高字重。当前行变化后自动滚动到卡片可视区域中央，首行与末行仍须可达。以上字号不应用于 07 DJ 串讲态。
