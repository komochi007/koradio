...
- 所有 path 保持确定且不随窗口尺寸重新计算；不使用随机数、逐帧重算或 resize 驱动的形态切换。允许三个线束层以 `10.8s`～`13.2s` 周期做低幅明暗呼吸，整体以约 `12s` 周期在 `1px`～`2px` 范围内缓慢漂移，并以约 `14s` 周期让宽幅渐变弧光流过原有曲面。动效只修改 transform/opacity 与 mask 位置，不改变 path，窗口缩放不得闪烁；Reduce Motion 下声场呼吸、漂移和弧光全部停止。
