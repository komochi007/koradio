...
错误色用于当前任务无法继续的状态，例如活动 AI Planner 未连接、音乐服务不可用或数据路径不可写。
...
Radio 大号时间优先使用本机 `HONOR Sans`，回退至系统无衬线字体；日期、状态标签和其它功能文字仍使用等宽或系统字体。大号时间不得回退为低可读性的装饰性点阵字体。

### 3.2 字体层级

| 层级           |   字号 |   行高 |  字重 | 主要用途                    |
| ------------ | ---: | ---: | --: | ----------------------- |
| Display Time | 80px | 80px | 900 | Radio 大号时间，`HONOR Sans Heavy` |
...
移动端 Display Time 调整为 `56px / 56px / 900`，H1 调整为 `28px`。
...
* Radio transcript 中每一条 DJ 与用户消息均使用克制、低密度的独立气泡，并分别配 `44px` 正圆头像；DJ 气泡用低浓度 accent-soft，用户气泡用 elevated surface
* Detail Sheet 中的 DJ 串讲仍采用逐句编辑排版，不沿用 Radio 的聊天气泡
...
响应式代表页使用 `390 × 844px`、`834 × 1194px`、`1440 × 1200px` 三组固定验收 viewport。`960 × 1600px` Prototype 继续使用 4.6、5.11 与 8.2 的固定坐标，不因响应式实现改变基线几何。

| Viewport | 页面安全边距 | 顶部安全边距 | 底部导航 |
|----------|--------------|--------------|----------|
| Mobile `390 × 844px` | 左右 `20px` | `24px` | 左右 `16px`、底部 `12px + safe area` |
| Tablet `834 × 1194px` | 左右 `28px` | `28px` | 最大宽度 `620px`、底部 `20px + safe area` |
| Desktop `1440 × 1200px` | 内容列外侧至少 `32px` | `32px` | 最大宽度 `620px`、底部 `24px + safe area` |

五类布局按以下规则由代表页继承：

* Profile：03 代表 02–03。顶部工具与表单内容列共享安全边缘；表单保持单列并独立纵向滚动。移动端只缩小固定比例头像与控件，不拆为两列表单，不隐藏字段、隐私说明或操作。
* Radio：05 代表 04–06。desktop standalone PWA 继续以 `960 × 1600px` 画布等比适配；Electron 日常紧凑窗口使用 `0.425` 固定内容比例，窗口继续缩窄时只收敛外侧留白，不再缩小组件、文字或改变布局。两者都不因窄窗口或浏览器缩放切换到 Mobile 骨架，页面外层不得滚动或隐藏内容。普通浏览器和手机继续使用响应式布局。顶部品牌与 Radio 内容区左边缘对齐，并在 Electron 最小窗口避让 macOS 原生按钮；底部场景输入和底部导航保持固定，队列与 DJ 对话等长内容只能在自身卡片内滚动且不显示滚动条。DJ 正文与用户气泡在紧凑窗口保持 `20px / 32px` 字号与行高。Tablet 与 Desktop 的 Radio rail 最大宽度保持 `816px`，宽屏不得增加侧栏或第二列。
* Detail：08 代表 07–08。Sheet 始终通过窗口根节点覆盖整个 viewport，外层与白色节目面左右不得保留画布边距；声场与节目面保持上下结构，节目内容列在 Mobile 使用页面安全边距，在 Tablet 与 Desktop 最大为 `848px`。状态、关闭、歌曲进度、主内容卡、节目进度和单一播放控制均不得移除。所有窗口尺寸下，状态行与关闭按钮共用顶部控制带并保持上下边界一致，状态行左端与节目内容列对齐，关闭按钮右端与节目面安全边缘对齐；Electron 最小窗口还需避让原生按钮。波形柱延续到节目面下方，由节目面自然覆盖中间区域并填满两侧圆角空隙，不使用色块或渐变补缝；关闭按钮、播放按钮和节目进度按窗口尺寸收敛，为歌词卡片保留更多高度。
* Management：10 代表 09–13。顶部工具、管理内容列与导航共享固定边缘；内容层独立滚动，最大宽度保持 `840px`。Desktop standalone PWA 与 Electron 仅在内容层滚动且隐藏滚动条，页面、画布和窗口均不得滚动。Library 的内容层可以整体滚动，但本地音乐列表仍须在自身卡片内独立滚动；Library、Taste 与 Programs 不显示主标题下的说明小字，Programs 的标题到内容区间距与其他管理页一致。移动端将同级并排卡片纵向排列，保持信息顺序，不使用后台侧栏。
* Settings：14 代表 14–15。顶部工具、底部操作与导航固定，设置内容独立滚动，最大宽度保持 `832px`。Desktop standalone PWA 的操作区固定在导航上方，内容区只在其上方滚动且隐藏滚动条，不得以底部留白填充画布。移动端仍保留状态文字、Codex 配置、内置 Provider 能力状态、偏好和本地数据操作，不把表单改成多栏或隐藏次级状态。

所有布局族共同遵循：

* 内容滚动区必须为固定顶部、底部操作与 safe area 预留空间，不得被导航遮挡。
* 长标题、中文说明、路径、状态和按钮文案不得造成横向滚动；允许在组件内部换行或省略低优先级路径显示，但可访问名称和交互语义必须保留。
* Mobile 可按共享比例收敛 Logo、头像、封面、导航选中底和播放器控件，但所有可用操作的命中区不得小于 `44 × 44px`。
* Dark、Light 与 System 只改变主题材质，不改变响应式节点顺序、尺寸或滚动边界。
...
--kr-radio-player-height: 340px;
--kr-radio-player-padding: 24px;
--kr-radio-dialogue-height: 288px;
...
Electron 紧凑窗口以 `900px` 逻辑宽度作为固定内容参考，内容比例上限为 `0.425`；当物理窗口继续缩窄时，优先减少 Radio rail 外侧留白，组件尺寸、文字字号、卡片内部间距和布局顺序保持不变。窗口策略最小值为 `430 × 652px`，任何页面不得通过缩小正文、隐藏组件或允许页面外层滚动来规避该限制。

Settings 服务配置页的 AI 大脑选择、Codex 命令路径输入框、DeepSeek 模型与 Keychain 密钥状态卡、持久隐私提示和内置 Provider 只读能力行固定为 `64px` 高；网易云行只显示“内置·本地模式”及健康状态，不得收集 API 地址、Cookie 或密钥，DeepSeek key 输入只允许写入 Keychain 且不得回显已保存值。已配置时只展示“已配置 + 编辑”，点击编辑才显示替换输入、删除和取消；删除使用 Koradio 确认对话框。该规格只作用于 Settings 页面族，不取代 Profile 表单和组件目录使用的全局 `80px` 输入规格。偏好设置的 Theme Mode、DJ Language 和 DJ Voice Style 控件统一为 `44px` 高，三行之间保留 `8px` 间距，不得让相邻控件边框直接相接。所有下拉选择使用 Koradio 自定义浮层组件并复刻原生键盘行为：触发器、选项的 hover/selected/disabled/focus 状态统一，支持方向键、Home/End、Enter/Space、Escape、Tab 和文字检索，浮层通过 portal 避免被滚动容器裁切；紧凑窗口下 chevron 的右端与服务状态 `AVAILABLE` 的右端对齐。服务状态中的 Test 与本地数据中的 Change 使用同一紧凑操作规格：真实命中区保持 `44px` 高，可见边框内收为 `32px` 高并垂直居中，边框上下各保留 `6px`，不得与外层卡片边框或分割线重叠。服务状态、服务配置、偏好设置、本地数据、固定操作区和底部导航之间统一保留 `24px` 的纵向间距。DJ Voice Style 选择器只显示当前值，不在选择框下重复显示声音风格说明。Profile 编辑页“我的头像”和“DJ 头像”各自使用独立的圆形裁图器，紧凑窗口下头像列按实际圆形尺寸收敛；档案选择页移除无语义箭头，`CURRENT` 与“编辑档案”共享右侧安全边缘。VDA-17 冻结原型中现有输入框式视觉槽位仅作像素基线，产品实现必须按本节语义将网易云槽位渲染为只读状态。
...
Radio 主页面的空状态、播放态和生成中状态必须共享同一套页面骨架。状态变化只替换中间内容，不改变内容 rail、组件宽度、底部输入框位置和底部导航位置。Queue 折叠是播放态内唯一允许重新分配垂直空间的交互：DJ 状态栏和对话区回收列表高度，对话区增高。
...
| 状态主内容区 | `x 72 / y 292 / w 816 / h 340` | 空状态、播放器或生成中核心内容在此替换 |
| 当前播放器卡片 | `816 × 340px` | 仅播放态显示，填满状态主内容区 |
| 队列区 / 骨架队列 | `x 72 / y 656 / w 816 / h 300` | 空、播放、生成中状态同宽 |
| DJ 状态栏 | `x 72 / y 980 / w 816 / h 64` | 状态文字变化不改变尺寸 |
| DJ 对话区 | `x 72 / y 1068 / w 816 / h 288` | 独立 dialogue well，三态同宽同高 |
...
| 组件 | 坐标与尺寸 | 约束 |
|------|------------|------|
| 折叠 Queue | `x 72 / y 656 / w 816 / h 60` | 只显示标题栏 |
| DJ 状态栏 | `x 72 / y 740 / w 816 / h 64` | 与 Queue 保持 `24px` |
| DJ 对话区 | `x 72 / y 828 / w 816 / h 520` | 与状态栏、场景输入均保持 `24px` |
...
* 展开态 DJ 状态栏、DJ 对话区、底部输入框、底部胶囊导航；折叠态按上表统一补位。
...
`Like` 的心形按钮只表达“喜欢当前歌曲”，再次操作表达“取消喜欢当前歌曲”，不得解释为歌曲收藏或节目收藏。`More` 承载“不喜欢当前歌曲”、查看来源等次级歌曲操作；节目收藏只在 Programs/节目入口处理。以上语义不改变冻结按钮位置、尺寸、图标或像素外观，且均不进入全屏 Detail Sheet。
...
Radio 当前播放器使用纵向等分布局：顶部媒体信息、进度条和底部控制区之间由剩余空间均匀分配；播放器内容与上、下边框均保持 `24px`，内部行距按同一空间比例收敛，不得通过单独的 `margin-top` 或 `margin-bottom` 形成不对称留白。
| Detail / 摘要卡片 | `24px` |
| 检测结果卡片 | `24px` |
...
Radio 内所有可操作按钮、可点击卡片操作和范围控件都使用系统手型光标，保持上述 Hover/Active 与可见键盘 Focus；禁用控件使用 `not-allowed`，不改变既有尺寸或布局。用户启用 `prefers-reduced-motion` 时保留状态与 Focus，关闭位移和缩放过渡。
...
播放态 Queue 从 `300px` 折叠为 `60px` 时，后续 DJ 区域必须使用同一 `320ms` standard easing 回收空间；展开时反向恢复，不使用弹跳或过冲。Reduce Motion 下直接切换最终布局。
...
DJ 对话区是 Profile 级持续对话与节目叙事的统一 transcript。它保留编辑感强的电台版式，不采用社交聊天应用的视觉语言；闲聊、澄清、单曲与最多 5 首策展卡片、节目生成进度和 DJ 串讲都在同一内部滚动区域连续呈现。

Radio 主页面原型规格：

* 坐标与尺寸：`x 72 / y 1068 / w 816 / h 288`
...
* 用户弱气泡最大宽度：`520px`
* 用户弱气泡圆角：`18px`
* Desktop / standalone PWA 对话正文：`20px / 32px`
* Mobile 对话正文：`16px / 26px`

内容规则：

* DJ 内容使用自然段排版，不进入气泡；最近 50 个 turn 在固定高度区域内滚动且不显示滚动条。
* DJ 段落下方可以显示 `22:46 · READ ALOUD` 等小型等宽元数据；节目串讲可继续提供 `REPLAY`。
* 用户原始输入使用右对齐弱气泡，气泡只比背景高一层亮度；弱气泡不降低文字层级，必须与 DJ 正文使用同一字号和行高。
* 歧义需求显示简短澄清问题，不启动音乐任务；单曲或多首策展推荐以紧凑纵向歌曲卡展示曲名、艺人、推荐理由与逐首 `PLAY NOW` / `PLAY NEXT`；节目生成状态作为 transcript 状态行展示，旧播放器与输入保持可用。
* 生成中状态先显示用户弱气泡，再显示 `Tuning your station...` 和灰色点阵；同一 Profile 已有生成任务时再次请求节目，先询问替换或继续等待。
* 对话中的 DJ 串讲不显示来源链接或 `SOURCE` 占位。
* 对话区不得出现机器人头像、连续胶囊式聊天气泡、社交聊天颜色、常驻输入建议按钮或大面积绿色背景。

三态展开 Queue 时必须保留对话区边界，即使空状态只有一句待命文案，也要显示同一块 `816 × 288px` dialogue well；播放态折叠 Queue 后扩展为 `816 × 520px`。
...
在 Electron 中，Detail Sheet 必须挂载到窗口根节点而非居中缩放画布内部；深色声场和白色节目面都覆盖完整物理窗口宽度，左右外边距为 `0`，仅节目内容列保留规定的内部安全边距。

Electron 下 Detail Sheet 使用实际 viewport，不继承 Radio、Library 等页面的 `960 × 1600px` 内容缩放。波形区与浅色节目面按约 `25% / 75%` 的窗口高度分配，波形柱在节目面上沿下方继续延伸约 `24px`，由上层节目面自然裁切中间部分并填充两侧圆角；节目面始终贴合窗口底部。最小 `430 × 652px` 窗口下，节目标题约 `26px` 起、歌曲信息约 `13px` 起，普通歌词约 `18px` 起，当前歌词约 `22px` 起。

规格：

* 全屏覆盖：`x 0 / y 0 / w 960 / h 1600`
* 顶部拖动条：`x 438 / y 32 / w 84 / h 6`
* 关闭按钮：`x 848 / y 40 / w 56 / h 56`
* DJ 状态区：`x 56 / y 40 / w 720 / h 56`，与关闭按钮保持同一控制带
* 声波区：`x 0 / y 160 / w 960 / h 280`
* 浅色节目面：`x 0 / y 420 / w 960 / h 1180`，顶部圆角 `56px`
* 节目面专属内容列：`x 56 / w 848`；该值仅用于 Detail Sheet，不改变其他页面族内容列
* 节目单名称：`x 56 / y 484 / w 848`，字号约 `40px`，行高 `43px`
* 歌曲信息：`x 56 / y 584 / w 848`
* 歌曲进度：`x 56 / y 636 / w 848 / h 20`
* 串讲词/歌词卡片：`x 56 / y 700 / w 848 / h 700`，圆角 `32px`；保留不少于 `32px` 的上下安全内距；07 串讲词从顶部开始排列，分句组间距 `24px`，按逗号、句号、等价句末标点和原始换行拆分；每个短句仅显示一个时间标识，英文只允许在完整单词之间换行；Detail 显示文本去除每句首尾标点，TTS 原始文本保持不变；08 歌词也从顶部开始排列，内部可滚动但不显示滚动条
* 节目整体进度条：`x 56 / y 1456 / w 784 / h 48`
* 单一播放/暂停按钮：约 `x 856 / y 1456 / w 48 / h 48`
* 声波区使用 `64` 根确定性竖条，顶点形成平滑包络；柱宽 `7px`，仅使用 `#AEB4B9`、`#CFD3D6`、`#F0F2F4` 三档白灰
* 节目整体进度使用确定性的非规则波形簇，不采用等周期或简单取模重复
...
当前句或当前歌词行使用浅色面主文本 `#14161A`；当前句中已播放字符继续保持主文本颜色，当前字符使用更强字重，并在字符/词的字体内部按播放进度从待播色平滑渐变到主文本色，避免整字瞬间跳变；未播放字符使用中灰 `#767A80`；句子结束后整句变为已读颜色 `#898A86`，在 `#F5F3F6` 大号正文卡片上保持 `3.15:1` 对比度。当前、已读与待播行通过透明度、颜色与 `transform: scale()` 平滑过渡，卡片只平滑滚动自身到当前行；避免使用大面积绿色文字高亮，可使用非常轻的词级绿色柔光或小状态点辅助定位，不使用绿色侧边条。

串讲当前句在放大前必须预留等比例行宽，缩放后只触及内容列边界，不得被卡片裁切；正文允许自然换行，但不得把英文单词或中文单字拆开。

若歌词或 DJ 串讲没有真实逐字时间戳，逐字进度按当前行的可读单元在该行时段内估算：中文按单个汉字推进，英文按完整单词推进，标点与空白跟随前一个单位。该估算只影响高亮进度，不改变 canonical timeline、播放位置或 TTS 原始文本。

歌词态普通、已读、待播和静态歌词使用 `clamp(18px, 2.7vw, 24px)`、`1.5` 行高；当前歌词使用 `clamp(24px, 3.5vw, 32px)`、`1.32` 行高并保留高字重。歌词卡片上下保留不少于 `32px` 的安全内距，当前行变化后只滚动歌词卡片自身到可视区域中央，首行与末行通过等量上下安全间距保持可达，不能推动外层页面。长歌词只能在英文单词间或中文单字间自然换行，并为放大后的当前行预留缩放安全宽度，不得被卡片边缘裁切、拆开单词或拆开中文单字。Electron 最小窗口使用 `clamp(18px, 3.6vw, 20px)` 普通歌词与 `clamp(22px, 4.4vw, 24px)` 当前歌词，顶部歌曲信息区进一步收紧；`prefers-reduced-motion` 下保留状态变化，关闭滚动与缩放动画。
...
所有瞬时 Toast 默认显示 `5000ms`，并提供手动关闭按钮。所有普通页面的 Toast 统一位于窗口顶部中央、标题栏安全区之下，使用半透明深色容器、状态色圆点、简短正文和圆形关闭按钮；成功使用绿色、错误使用红色、一般状态使用中性灰。真正阻断播放或生成的问题不使用自动消失的 Toast，必须保留在播放器或对话区并提供恢复动作。

Library 的“本地音乐”内滚动列表在分页加载时，加载状态占用列表区预留的固定位置，不得插入或移除正常文档流以推动下方的歌单导入和本地候选池卡片。

常规确认、配置和编辑对话框统一使用全屏低饱和遮罩与背景模糊，前景卡片在可视窗口中心对齐，并共用深色表面、细边框、`24px` 圆角和右对齐操作区；危险操作保留红色状态标签及危险按钮。全屏 Radio Detail Sheet 是沉浸式详情容器，不适用该常规对话框规格。

阻断错误应在相关区块内联显示，不能只使用 Toast。例如活动 Planner 未配置时，应在场景输入区显示错误和 Settings 入口；DeepSeek 首次启用还必须先确认隐私提示。

队列标题栏的 `HIDE/LIST` 必须是可聚焦按钮，使用 `aria-expanded` 表示列表状态；新节目默认展开，折叠不影响播放。

Radio、Library 和 Settings 的运行标识固定为 `LIVE` 或 `DEMO MODE`。Programs 中 Mock 历史卡片显示 `DEMO` 标签。节目卡片与详情更多菜单提供“删除节目”，确认对话框显示节目标题、不可恢复说明和明确的危险操作按钮。
...
* 使用柔和的人声式波形
* 64 根柱体的顶点由确定性控制点生成平滑包络，柱宽 `7px`，移动端收敛为 `3px`
* 柱体只使用白色与浅灰色三档明度，不使用绿色；绿色只保留在状态点
* 动画周期：`880ms`，相邻柱体使用 `13.75ms` 相位差形成同向传播的波浪流动，缩放范围约 `0.78–1`
* 当前句切换时不做闪烁

#### PLAYING

* 常规播放组件使用 12–24 根竖条；Detail Sheet 使用 64 根竖条
* Detail Sheet 与 SPEAKING 共用平滑包络、白色/浅灰配色和传播式波浪动效
* 不要求与真实频谱完全同步
* 最大高度差不超过容器高度的 70%
...
* 页面切换后将焦点移至新页面主标题；关闭 Detail Sheet 后将焦点返回 DJ 状态栏。
* 底部导航支持 Tab，并允许使用左右方向键、Home 与 End 在五项之间移动焦点。
* Detail Sheet 打开后焦点进入关闭按钮，Tab 焦点保持在 Sheet 内，Esc 关闭且播放不中断。
* 加载和非阻断状态使用 `role="status"` 与 polite `aria-live`；阻断错误使用 `role="alert"` 或 assertive `aria-live`，不得让整个页面容器重复播报。
* Disabled、Loading、Error 与 Paused 必须同时提供文字或可访问名称，不依赖透明度、动画或颜色单独表达。
...
Reduce Motion 下，持续波形、骨架闪烁、状态脉冲、页面淡入位移和 Detail Sheet 上下转场均停止；页面层级、焦点顺序、状态文字和播放控制保持不变。
...
* 一级页面底部使用固定胶囊导航，规格为 `620 × 88px`，距离画布底部 `32px`；选中态使用白色圆角矩形底和纯黑图标，不得因断点变为圆形或被主题样式覆写。
* Radio 场景输入右侧的语音与发送操作在同一布局族中必须使用相同的圆角正方形外框：Desktop `64 × 64px / 16px` 圆角，Mobile `48 × 48px / 14px` 圆角。
...
* 图片和专辑封面只作为小尺寸识别信息，不抽取背景色，不主导页面视觉；图片加载失败时使用中性占位图。
...
| 04 | Radio 空节目状态 | `x 72 / w 816` Radio rail；输入框 `x 72 / y 1372 / w 816 / h 88` | `● LIVE`，不显示 `ON AIR`；导航选中 Radio | 固定顶部品牌与头像、`HONOR Sans Heavy` 80px 时间区、`NOW PLAYING`、`NO SESSION ON AIR`、`QUEUE · 0 TRACKS`、`816 × 64px` DJ 状态栏、`816 × 288px` dialogue well、固定场景输入框；输入框下方不显示状态文字 |
| 05 | Radio 正在播放 | 与 04 共用同一套 Radio slot；播放器、队列、DJ 状态栏、dialogue well 和输入框同宽 | `● ON AIR`；DJ 状态 `PLAYING`；导航选中 Radio | `816 × 340px` 当前播放器、24px 内边距、132px 小封面、歌曲信息、进度 `01:42 / 02:35`、96px 圆形暂停按钮、816px 队列 4 首、当前曲目 2px 绿色状态线、DJ 自然段串讲、用户弱气泡、固定场景输入框 |
...
| 07 | Detail Sheet：DJ 串讲 | `x 0 / y 0 / w 960 / h 1600` 全屏沉浸节目界面；浅色节目面内使用 Detail 专属 `x 56 / w 848` 内容列 | `● SPEAKING NOW`；无底部导航 | 顶部 `84 × 6px` 拖动条、`64 × 64px` 关闭按钮、64 柱白灰平滑包络声波、880ms 传播式波浪、纯白节目面、52px 节目标题、`If · Bread`、歌曲进度、顶部信息区紧凑排列、`848 × 700px` 串讲词卡片、非规则节目整体进度、单一播放/暂停 |
| 08 | Detail Sheet：歌词跟随 | 与 07 共用同一套全屏固定骨架 | `● PLAYING`；无底部导航 | 64 柱白灰平滑包络声波、880ms 传播式波浪、纯白节目面、52px 节目标题、`Space Song · Beach House`、歌曲进度、`848 × 700px` 歌词卡片、当前歌词高亮、非规则节目整体进度、单一播放/暂停 |
| 09 | Library 音乐库 | `840px` 单列管理页 | 导航选中 Library | 标题“音乐库”、副标题、本地音乐数量、搜索框、5 条搜索结果、试听图标、加入候选池按钮、导入网易云歌单卡片、运行模式 `LIVE` 或 `DEMO MODE`、已导入来源 |
| 10 | Taste 品味档案 | `840px` 单列管理页 | 导航选中 Taste | 标题“你的音乐品味”、编辑品味按钮、品味概览摘要、品味蓝图卡片（来源、核心特质、审美簇、70/30 比例、歌曲语言比例、版本选择偏好）、常听风格标签、喜欢的声音、避雷规则、场景偏好、重塑后最近反馈；重塑确认使用紧凑前景确认卡 |
...
8. DJ 对话区是否保留 `816 × 288px` dialogue well 边界，DJ 文案是否为自然段，用户输入是否为右对齐弱气泡。
9. 主按钮是否保持黑白高对比；绿色是否只用于状态点、焦点环或细线，Detail 顶部声波是否仅使用白色与浅灰色。
...
## 10. UX-30 Daily Mix 界面规范

Daily Mix 采用克制的低频声场编辑感，服从既有 Radio `816px` 中央 rail、Dark Theme 表面与单一绿色状态色。它是承载 20 首可操作列表的任务型模态层，不使用玻璃拟态、霓虹、厚重阴影、渐变文字或彩色封面拼贴。

### 10.1 入口与模态层

- Radio 工具区新增不带日期数字的极简日历按钮，点击区域不得小于 `44 × 44px`。今日歌单成功后日历线条使用既有 accent；生成中、失败和无结果使用中性文字色，不增加状态点。
- 遮罩覆盖物理 viewport，使用低饱和黑色压暗与克制背景模糊。卡片在可视区水平、垂直居中，宽度不超过 `760px`，左右至少保留 `24px`，高度不超过 `calc(100vh - 48px)`，圆角 `24px`，细边框，不使用重阴影。
- 顶部和列表标题固定，歌曲区独立滚动。紧凑 Electron 窗口只压缩顶部声场高度与间距，不删除日期、播放全部、关闭按钮或歌曲操作。
- 关闭按钮、`Esc` 和遮罩点击均关闭；关闭不影响播放。打开后焦点进入关闭按钮并限制在模态层，关闭后回到日历入口。

### 10.2 Daily Mix 卡片

- 顶部左侧依次为 `DAILY MIX`、大号 `MM/DD` 和“播放全部”；右侧为无独立容器的低频参数声场。日期使用等宽重字重，其他英文标签全部大写。
- 声场作为 Header 的背景图形层，不设置独立插图容器或可感知的矩形边界。默认使用 `DailyMixWaveMesh` 生成 72 条 `0.72px` 的确定性 SVG 细线；允许在 48～72 条、`0.6px`～`1px` 范围内调节。三组线束共享 2～3 个低频峰谷，又以错开的峰位、相位和局部弯曲形成前后交错的连续曲面，不得呈现均匀正弦、碎波、单根高亮线、均衡器、频谱或 HUD。
- 声场只使用黑白灰，通过少量主结构线、中层主体线、外围环境线和三层低透明曲面建立明暗层次；高亮只能以宽幅柔和 mask 让原有线面局部变亮，不得叠加独立亮线、绿色节点、彩色渐变、霓虹或重发光。SVG 内部水平/垂直 mask 与容器响应式 alpha mask 共同让左、右、上、下自然渐隐；左侧必须经过两段以上透明度衰减后再进入主体亮度，右侧允许在完成渐隐后被容器裁切。Desktop 主体宽度约为 Header 的 `92%`，紧凑窗口约为 `112%` 并向容器外延伸，通过可视范围变化而非横向压缩适配。
- 所有 path 保持确定且不随窗口尺寸重新计算；不使用随机数、逐帧重算或 resize 驱动的形态切换。三个线束层以 `10.8s`～`13.2s` 周期做低幅明暗呼吸，整体以约 `12s` 周期只做 `0.96`～`1` 的透明度呼吸，不得平移、缩放或重采样细线。弧光使用低矮椭圆径向遮罩，以 `9.6s` 匀速改变 `cx` 横向流过三层连续曲面柔光，右半段允许轻微增强；不得复制 72 条细线作为弧光层，以避免抗锯齿叠加、莫尔闪烁和亚像素抖动。Reduce Motion 下声场呼吸、线束明暗和弧光全部停止。
- `PLAY ALL` 使用 `18 × 18px` 黑色正圆和居中白色三角图标，图标与文字在同一水平中心线上。
- 分隔线下方使用 `TODAY'S PICKS · 20 TRACKS`。单行固定为序号、方形封面、歌曲名、艺术家、圆形播放按钮和圆形下一首按钮；不显示专辑、时长或推荐理由。
- 标题与艺术家按同一左边线对齐，操作按钮按同一右边线对齐；文本溢出单行截断并提供完整可访问名称。当前歌曲使用克制的细均衡器、主文字和轻微表面差，不使用整行绿色填充；圆角高亮表面与卡片左右留白一致，歌曲分隔线维持固定长度，当前行下方分隔线紧贴圆角表面的外缘。
- 生成中只显示当日生成状态，不展示旧列表；失败显示“今日推荐暂未生成”和重新生成；结束显示“今日推荐已播完”并保留返回原节目。

### 10.3 Queue 来源 Switch

- Queue 标题的 `TRACKS` 右侧放置 `128 × 28px`（紧凑宽度 `120 × 28px`）的双段胶囊，仅包含 `RADIO` 与 `DAILY` 文字。它是单一深色母胶囊：选中态为内缩白色滑块，仅在母胶囊外缘保留均等黑色留白；未选中一侧不出现独立边框、分隔线或浅色描边。文字在各自可用区域内居中，整体与标题栏垂直中心精确对齐。
- 滑块表示当前基础来源，使用 220ms `cubic-bezier(0.2, 0, 0, 1)` 横向 transform；禁止使用图标、状态点或大面积绿色。无目标来源时对应方向不可点击并降低对比度。
- 胶囊支持 Tab 聚焦与点击，使用 `aria-label` 与 `aria-pressed` 表达来源；临时插播期间仍显示基础来源。

### 10.4 Daily Detail 与历史

- Daily 播放时 Queue 标题为 `DAILY MIX · 20 TRACKS`，DJ 状态为 `DAILY MIX · PLAYING`，原 DJ 对话区保持可用。
- Detail 沿用全屏沉浸骨架，标题固定为 `DAILY PROGRAM`，不显示日期、中文标题、DJ 串讲或场景内容。
- Programs 将成功 Daily 与 Program 按时间混排。Daily 卡片使用 `DAILY` 类型标识和次要日期信息；详情只提供播放全部、歌曲列表、逐首播放/下一首和单曲反馈，不显示整份收藏、更多删除、DJ 重播或场景复用。

## 11. UX-31 Programs 与 Radio 交互规范

### 11.1 Programs 间距与可点击性

- Programs 内容区使用统一的布局变量：相邻区块间距为 `24px`，节目卡片列表间距为 `16px`。不得以单张卡片的额外外边距修补局部节奏。
- `DAILY PROGRAMS` 小卡片和节目历史卡片均为可点击对象，悬停时必须使用手型指针；键盘焦点仍需保留清晰可见的焦点样式。

### 11.2 Radio 播放进度

- 进度轨道默认高度为 `3px`；可操作状态下悬停或键盘聚焦时，以 `180ms ease-out` 平滑增高至 `6px`，不得突然缩放整个播放器。
- 交互态在绿色已播放段尾端显示 `10 × 10px` 圆形拖动控件，并以透明度和缩放过渡进入；禁用态保持原尺寸且不显示交互强化。
