...
当前句或当前歌词行使用浅色面主文本 `#14161A`；当前句中已播放字符继续保持主文本颜色，当前字符使用更强字重，未播放字符使用中灰 `#767A80`；句子结束后整句变为已读颜色 `#898A86`，在 `#F5F3F6` 大号正文卡片上保持 `3.15:1` 对比度。避免使用大面积绿色文字高亮，可使用非常轻的词级绿色柔光或小状态点辅助定位，不使用绿色侧边条。

若歌词或 DJ 串讲没有真实逐字时间戳，逐字进度按当前行的可读文字长度在该行时段内估算：中文按单个汉字推进，英文按单词推进，标点与空白跟随前一个单位。该估算只影响高亮进度，不改变 canonical timeline、播放位置或文本内容。

歌词态普通、已读、待播和静态歌词使用 `clamp(24px, 3.75vw, 36px)`、`1.55` 行高；当前歌词使用 `clamp(34px, 5vw, 48px)`、`1.35` 行高并保留高字重。当前行变化后只滚动歌词卡片自身到可视区域中央，首行与末行通过等量上下安全间距保持可达，不能推动外层页面。以上字号不应用于 07 DJ 串讲态；Electron 最小窗口使用独立的紧凑字号下限。
...
| 14 | Settings 服务配置 | `832px` 单列设置页；Codex 配置与 Provider 能力行 `64px` 高；主要区块间距 `24px` | `● 3 SERVICES ONLINE`；导航选中 Settings | 标题“设置”、服务状态四行、TTS `DEGRADED` 黄色状态、Codex 命令路径输入框、网易云“内置·本地模式”只读行、Theme Mode、DJ Language、DJ Voice Style、本地数据、底部固定操作区；不显示网易云地址或密钥控件，声音风格选择器下方不显示重复说明 |
