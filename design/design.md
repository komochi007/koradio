...
* 串讲词/歌词卡片：`x 56 / y 700 / w 848 / h 700`，圆角 `32px`；保留不少于 `32px` 的上下安全内距；07 串讲词从顶部开始排列，分句组间距 `24px`，按句末、自然停顿和原始换行优先拆分，中文超长句可在约 `18–24` 字处拆开，英文只允许在完整单词之间换行，收尾标点必须跟随前文；08 歌词也从顶部开始排列，内部可滚动但不显示滚动条
...
串讲当前句在放大前必须预留等比例行宽，缩放后只触及内容列边界，不得被卡片裁切；正文允许自然换行，但不得把英文单词或收尾标点拆到下一行。

若歌词或 DJ 串讲没有真实逐字时间戳，逐字进度按当前行的可读文字长度在该行时段内估算：中文按单个汉字推进，英文按单词推进，标点与空白跟随前一个单位。该估算只影响高亮进度，不改变 canonical timeline、播放位置或文本内容。

歌词态普通、已读、待播和静态歌词使用 `clamp(18px, 2.7vw, 24px)`、`1.5` 行高；当前歌词使用 `clamp(24px, 3.5vw, 32px)`、`1.32` 行高并保留高字重。歌词卡片上下保留不少于 `32px` 的安全内距，当前行变化后只滚动歌词卡片自身到可视区域中央，首行与末行通过等量上下安全间距保持可达，不能推动外层页面。长歌词允许按词内断行，并为放大后的当前行保留缩放安全宽度，不得被卡片边缘裁切。Electron 最小窗口使用 `clamp(18px, 3.6vw, 20px)` 普通歌词与 `clamp(22px, 4.4vw, 25px)` 当前歌词，顶部歌曲信息区进一步收紧；`prefers-reduced-motion` 下保留状态变化，关闭滚动与缩放动画。
