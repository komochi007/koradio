...
DJ 对话区是 Profile 级持续对话与节目叙事的统一 transcript。它保留编辑感强的电台版式，不采用社交聊天应用的视觉语言；闲聊、澄清、单曲与最多 5 首策展卡片、节目生成进度、DJ 串讲与来源都在同一内部滚动区域连续呈现。
...
* 可信背景事实下方显示紧凑来源链接；无可信来源时不展示来源占位。
...
* 串讲词/歌词卡片：`x 56 / y 700 / w 848 / h 700`，圆角 `32px`；保留不少于 `32px` 的上下安全内距；07 串讲词从顶部开始排列，分句组间距 `24px`，按句末、自然停顿和原始换行优先拆分，中文超长句可在约 `18–24` 字处拆开，英文只允许在完整单词之间换行；Detail 显示文本去除每句首尾标点，TTS 原始文本保持不变；08 歌词也从顶部开始排列，内部可滚动但不显示滚动条
