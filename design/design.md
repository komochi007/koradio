...
普通浏览器响应式代表页使用 `390 × 844px`、`834 × 1194px`、`1440 × 1200px` 三组固定验收 viewport。`960 × 1600px` Prototype 继续使用 4.6、5.11 与 8.2 的固定坐标，不因运行时响应式实现改变基线几何。Desktop standalone PWA 使用 `1440 × 801px`、`960 × 720px` 与 `720 × 800px` 三组内容验收 viewport，并单独模拟低于 `680 × 760px` 的外窗提示。
...
* Radio：05 代表 04–06。Desktop standalone PWA 使用最大 `720px` 宽、`100dvh` 高的中央单列运行时画布；时间、日期、状态纵向排列，播放器歌曲与来源信息严格单行。顶部品牌、底部场景输入和底部导航固定，Queue 与 DJ 对话只在自身卡片内滚动。
...
* Management：10 代表 09–13。Desktop standalone PWA 的标题与主要操作固定，集合在明确卡片内滚动，运行时内容最大宽度 `720px`。Library 本地音乐卡片高度约 `260–340px`，隐藏滚动条并在距底部约 `160px` 时自动加载下一页。
* Settings：14 代表 14–15。顶部工具、标题、底部操作与导航固定，长表单在独立内容面板内滚动；运行时内容最大宽度 `720px`，移动端仍保留全部配置和状态。
...
launcher 默认创建 `720 × 840px` 外窗。任一外窗维度低于 `680 × 760px` 时只显示可访问的窗口过小提示，不继续压缩业务界面；判断使用 `window.outerWidth / outerHeight`，200% 页面缩放不触发该提示。

运行时画布水平居中、不设置固定宽高比、不使用 `transform: scale()`。正文使用 `15–16px`，辅助文字 `12–13px`，页面标题 `32–40px`，Radio 时钟 `52–56px`，交互目标不小于 `44px`。页面外层不可滚动，只有 Queue、DJ 对话、数据集合、长表单和 Detail 文本卡片允许局部滚动。
...
* 用户弱气泡最大宽度：`520px`
* 用户弱气泡圆角：`18px`
* Desktop / standalone PWA 对话正文：`16px / 24px`
...
当前句或当前歌词行使用浅色面主文本 `#14161A`；句内当前 token 同时使用高字重、轻量绿色柔光和底部细线，已读 token 使用 `#898A86`，未播放 token 使用 `#767A80`，不得只依赖颜色。精确 Provider/TTS token 时间优先；缺失时按行或句窗口和可读字符权重估算，并显示 `Approximate word timing`。视觉 token 不逐词触发屏幕阅读器播报，完整行文本仍保持可访问。
