...
普通浏览器响应式代表页使用 `390 × 844px`、`834 × 1194px`、`1440 × 1200px` 三组固定验收 viewport。`960 × 1600px` Prototype 继续使用 4.6、5.11 与 8.2 的固定坐标，不因运行时响应式实现改变基线几何。Desktop standalone PWA 另使用 `1440 × 801px`、`960 × 720px`、`720 × 650px` 与 `560 × 600px` 四组运行时验收 viewport。
...
* Radio：05 代表 04–06。Desktop standalone PWA 使用最大 `960px` 宽、`100dvh` 高的中央单列运行时画布，不整体缩放，也不因窄窗口切换到 Mobile 骨架；页面外层不得滚动。顶部品牌、底部场景输入和底部导航保持固定，Queue 列表与 DJ 对话正文只能在自身卡片内滚动。Tablet 与 Desktop 的 Radio rail 最大宽度保持 `816px`，宽屏不得增加侧栏或第二列。
* Detail：08 代表 07–08。Sheet 始终覆盖整个 viewport；声场与节目面保持上下结构，节目内容列在 Mobile 使用页面安全边距，在 Tablet 与 Desktop 最大为 `848px`。状态、关闭、歌曲进度、主内容卡、节目进度和单一播放控制均不得移除。
* Management：10 代表 09–13。顶部工具、管理内容列与导航共享固定边缘；内容层独立滚动，最大宽度保持 `840px`。Desktop standalone PWA 仅在内容层滚动且隐藏滚动条，页面、画布和浏览器窗口均不得滚动。移动端将同级并排卡片纵向排列，保持信息顺序，不使用后台侧栏。
...
### 4.2.2 Desktop standalone PWA 运行时画布

高保真原型画布与运行时 PWA 画布使用不同约束。`960 × 1600px` 只负责原型出图和历史 VDA 对照；运行时 PWA 使用：
...
运行时画布水平居中、不设置固定宽高比、不使用 `transform: scale()`。`html`、`body`、`#root`、画布和页面外层均不可滚动。窗口高度降低时使用连续的间距、封面和非文字组件尺寸，不缩放字体；正文保持 `18px`、辅助文字 `16px`、状态标签 `14px`、DJ 正文 `20px`，交互目标不小于 `44px`。

Desktop standalone PWA 始终继承同一中央单列布局族。窄窗口允许内容列变窄、文本换行和组件内部重排，但不得切换为 Mobile 页面骨架。Radio 的 Queue 列表与 DJ 对话正文、Profile / Management / Settings 的主内容层、Detail 的歌词或串讲卡片是唯一允许的滚动边界，且必须支持指针、键盘与 200% 浏览器缩放。
