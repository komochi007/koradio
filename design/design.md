...
Settings 服务配置页的 AI 大脑选择、Codex 命令路径输入框、DeepSeek 模型与 Keychain 密钥状态卡、持久隐私提示和内置 Provider 只读能力行固定为 `64px` 高；网易云行只显示“内置·本地模式”及健康状态，不得收集 API 地址、Cookie 或密钥，DeepSeek key 输入只允许写入 Keychain 且不得回显已保存值。该规格只作用于 Settings 页面族，不取代 Profile 表单和组件目录使用的全局 `80px` 输入规格。偏好设置的 Theme Mode、DJ Language 和 DJ Voice Style 控件统一为 `44px` 高，三行之间保留 `8px` 间距，不得让相邻控件边框直接相接。所有下拉选择保留原生键盘与选择行为，隐藏浏览器默认箭头并使用右侧留有安全间距的统一 chevron；紧凑窗口下 chevron 的右端与服务状态 `AVAILABLE` 的右端对齐。服务状态中的 Test 与本地数据中的 Change 使用同一紧凑操作规格：真实命中区保持 `44px` 高，可见边框内收为 `32px` 高并垂直居中，边框上下各保留 `6px`，不得与外层卡片边框或分割线重叠。服务状态、服务配置、偏好设置、本地数据、固定操作区和底部导航之间统一保留 `24px` 的纵向间距。DJ Voice Style 选择器只显示当前值，不在选择框下重复显示声音风格说明。Profile 编辑页“头像”标签与头像容器之间保留明确的垂直间距，紧凑窗口下头像列按实际圆形尺寸收敛；档案选择页移除无语义箭头，`CURRENT` 与“编辑档案”共享右侧安全边缘。VDA-17 冻结原型中现有输入框式视觉槽位仅作像素基线，产品实现必须按本节语义将网易云槽位渲染为只读状态。
