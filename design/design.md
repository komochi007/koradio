...
| 层级           |   字号 |   行高 |  字重 | 主要用途                    |
| ------------ | ---: | ---: | --: | ----------------------- |
| Display Time | 56px |  1.0 | 500 | Radio 大号时间              |
| H1           | 32px | 40px | 600 | 页面主标题、节目标题              |
| H2           | 24px | 32px | 600 | 页面分区、详情标题               |
| H3           | 18px | 26px | 600 | 卡片标题、设置区标题              |
| Body Large   | 17px | 26px | 400 | DJ 串讲、重点正文              |
| Body         | 15px | 23px | 400 | 常规正文、表单文字               |
| Body Small   | 13px | 20px | 400 | 辅助描述、列表元数据              |
| Caption      | 11px | 16px | 500 | 状态、时间、标签                |
| Label Mono   | 11px | 16px | 600 | `ON AIR`、`QUEUE`、`LIVE` |
...
--kr-radio-width-tablet: 520px;
--kr-radio-width-desktop: 560px;

--kr-profile-width: 720px;
--kr-management-width: 800px;
--kr-settings-width: 820px;
...
|----------|----------|----------------|----------|
| Radio / 服务异常 | `560px` | 左右各 `200px` | 服务未连接、Radio 空状态、Radio 播放、节目生成中 |
| Profile / Onboarding | `720px` | 左右各 `120px` | 档案选择、创建档案 |
| 管理页 | `800px` | 左右各 `80px` | Library、Taste、Programs、历史节目详情 |
| Settings | `820px` | 左右各 `70px` | 服务配置、服务检测结果 |
| Detail Sheet | `960px` 全屏覆盖；浅色节目面内使用 `800px` 内容列 | 外层不留底层页面；浅色面内左右各 `80px` | DJ 串讲态、歌词跟随态 |
...
| 层级 | 宽度与位置 | 内容 |
|------|------------|------|
| 顶部品牌工具区 | `x 56 / y 56 / w 848 / h 48` | Logo、KORADIO、用户头像、主题切换 |
| Radio 内容 rail | `x 200 / w 560`，垂直贯穿画布 | 时间状态区、播放器/空状态/生成区、队列、DJ 状态栏、DJ 对话区、底部输入框 |
...
| 组件 | 坐标与尺寸 | 约束 |
|------|------------|------|
| 时间状态区 | `x 200 / y 136 / w 560 / h 132` | 三态保持同一位置、同一字号 |
| 状态主内容区 | `x 200 / y 320 / w 560 / h 220` | 空状态、播放器或生成中核心内容在此替换 |
| 当前播放器卡片 | `560 × 220px` | 仅播放态显示，填满状态主内容区 |
| 队列区 / 骨架队列 | `x 200 / y 592 / w 560 / h 256` | 空、播放、生成中状态同宽 |
| DJ 状态栏 | `x 200 / y 888 / w 560 / h 56` | 状态文字变化不改变尺寸 |
| DJ 对话区 | `x 200 / y 976 / w 560 / h 284` | 独立 dialogue well，三态同宽同高 |
| Radio 场景输入框 | `x 200 / y 1400 / w 560 / h 72` | 固定在底部 Tab 栏上方 |
| 底部胶囊导航 | `x 300 / y 1504 / w 360 / h 64` | 五个等宽 tab slot |
...
| 状态主内容区 | 空状态标题和引导文案 | `560 × 220px` 播放器 | 生成标题、说明、`400 × 56px` 中性色波形 |
...
* 原型图标尺寸：`24 × 24px`
* 工程默认图标尺寸：`20px`
* 原型 Tab slot：5 个等宽 `72 × 64px`
* 原型选中底：`56 × 48px`，位于当前 tab slot 中心
* 单项最小点击区域：`44 × 44px`
* 原型画布底部距离：`32px`
...
| 元素 | 尺寸 | 形状 | 约束 |
|------|------|------|------|
| 档案列表头像 | `64 × 64px` | 标准圆形 | 只显示圆形头像本身，不能随档案卡片高度拉伸 |
| 创建档案头像上传 | `88 × 88px` | 标准圆形 | 默认图形居中，编辑按钮可叠在右下角 |
| 顶部用户头像 | `36 × 36px` | 标准圆形 | 与主题按钮同属顶部工具区，不放大成装饰图 |
| 歌曲列表小封面 | `48 × 48px` | 圆角方形 | `10–12px` 圆角，不裁成圆形 |
| Radio 当前封面 | `56 × 56px` | 圆角方形 | 不抽取背景色，不被播放器卡片拉伸 |
| Detail Sheet 封面 | 不使用 | 不使用 | Detail Sheet 仅显示简洁歌曲文字，不显示专辑封面 |
| 图标按钮 | `44 × 44px` | 圆形或小圆角 | 点击区域固定，不因图标大小变化 |
| 主播放按钮 | `56 × 56px` | 标准圆形 | 播放/暂停状态切换不改变尺寸 |
| 状态点 | `6 × 6px` 或 `7 × 7px` | 标准圆形 | 不随文字行高变成椭圆 |
...
| 元素 | 原型坐标与尺寸 | 视觉规则 |
|------|----------------|----------|
| 顶部品牌工具区 | `x 56 / y 56 / w 848 / h 48` | 一级页面默认顶部工具行；二级页面可只使用其中的返回按钮和字标 |
| Logo 图标 | `x 56 / y 62 / w 36 / h 36` | 单色几何低细节广播图标 |
| KORADIO 字标 | `x 108 / y 65 / w 160 / h 30` | `18px`，`600`，字距 `0.12em` |
| 顶部用户头像 | `x 804 / y 62 / w 36 / h 36` | 标准正圆，不显示额外底板 |
| 主题切换按钮 | `x 860 / y 58 / w 44 / h 44` | 标准圆形图标按钮 |
| 顶部设置按钮 | `44 × 44px` | 与主题切换按钮同尺寸，使用同一图标按钮样式 |
| 顶部返回按钮 | `44 × 44px` | 位于内容列左侧或顶部工具行左侧，图标视觉中心对齐 |

Logo 图标与字标共同构成一个品牌整体，不允许在不同页面或不同状态中改变图标尺寸、字标字号、字间距或二者间距。头像、主题按钮、设置按钮和返回按钮属于同一顶部工具组件族，不允许被模型放大成装饰元素，也不允许缩小成普通列表图标。

如果品牌 lockup 出现在 `720px`、`800px` 或 `820px` 内容列内，组件尺寸仍保持不变，只改变整体 x 坐标以贴合当前内容列左边缘。若只显示 `KORADIO` 字标而不显示 Logo，字标仍使用 `18px`、`600`、`0.12em`，高度保持 `30px`，不得生成新的小号品牌字标。
...
* 尺寸：`560 × 72px`
* 位置：`x 200 / y 1400 / w 560 / h 72`，底部距画布 `128px`
* 内边距：左右 `24px`
* 右侧控件：语音图标区 + `48 × 48px` 圆形发送按钮
...
档案卡片高度推荐 `152px`，头像、档案信息和右侧箭头垂直居中。当前档案的 `CURRENT` 标签放在右侧操作区顶部，不再显示最近使用时间。
...
Radio 主页面中的队列区宽度固定为 `560px`，必须与播放器、DJ 状态栏和底部输入框左右对齐。
...
三态必须保留对话区边界，即使空状态只有一句待命文案，也要显示同一块 `560 × 284px` dialogue well。
...
* 关闭按钮：`x 864 / y 40 / w 56 / h 56`
* DJ 状态区：`x 80 / y 76 / w 720 / h 40`
* 声波区：`x 0 / y 160 / w 960 / h 280`
* 浅色节目面：`x 0 / y 420 / w 960 / h 1180`，顶部圆角 `56px`
* 节目面内容列：`x 80 / w 800`
* 串讲词/歌词卡片：`x 80 / y 760 / w 800 / h 560`，圆角 `32px`
* 节目整体进度条：`x 80 / y 1456 / w 680 / h 48`
* 单一播放/暂停按钮：约 `x 824 / y 1452 / w 56 / h 56`
...
* 全局品牌和工具组件尺寸固定：Logo `36 × 36px`，`KORADIO` 字标 `160 × 30px`，顶部头像 `36 × 36px`，顶部图标按钮 `44 × 44px`，状态点默认 `7 × 7px`，底部 Tab Bar `360 × 64px`，tab slot `72 × 64px`，图标 `24 × 24px`，选中底 `56 × 48px`。
* 一级页面底部使用固定胶囊导航，规格为 `360 × 64px`，距离画布底部 `32px`。
* 主按钮使用黑白高对比，不使用绿色填充。绿色只用于 `ON AIR`、`LIVE`、`PLAYING`、`CONNECTED` 等状态点或细线。
* Radio 类页面保持 `x 200 / w 560` 中央单列，不拆分左右栏；管理类页面保持单列，不做后台式侧边栏。
...
| 02 | 本地档案选择 | `720px` 中央内容列；三张纵向档案卡片；卡片统一 `24px` 内边距 | 第一张卡片 `CURRENT`；右上角可放设置图标 | 全局品牌 lockup、标题“选择你的电台档案”、`64 × 64px` 标准圆形头像、档案名/昵称/品味标签、虚线创建卡片、本地存储说明；不显示最近使用时间 |
| 03 | 创建电台档案 | `720px` 单列表单 | 返回箭头；保存后进入 Radio | `88 × 88px` 标准圆形头像上传、电台名称、昵称、常听风格标签、默认场景多行输入、主按钮“保存并进入 Koradio”、Ghost 按钮“稍后设置偏好”、本地隐私说明 |
| 04 | Radio 空节目状态 | `x 200 / w 560` Radio rail；输入框 `x 200 / y 1400 / w 560 / h 72` | `● LIVE`，不显示 `ON AIR`；导航选中 Radio | 固定顶部品牌与头像、固定时间区、`NOW PLAYING`、`NO SESSION ON AIR`、`QUEUE · 0 TRACKS`、`560 × 56px` DJ 状态栏、`560 × 284px` dialogue well、固定场景输入框；输入框下方不显示状态文字 |
| 05 | Radio 正在播放 | 与 04 共用同一套 Radio slot；播放器、队列、DJ 状态栏、dialogue well 和输入框同宽 | `● ON AIR`；DJ 状态 `PLAYING`；导航选中 Radio | `560 × 220px` 当前播放器、56px 小封面、歌曲信息、进度 `01:42 / 02:35`、56px 圆形暂停按钮、560px 队列 4 首、当前曲目 2px 绿色状态线、DJ 自然段串讲、用户弱气泡、固定场景输入框 |
| 06 | Radio 生成节目中 | 与 04 共用同一套 Radio slot；骨架队列、DJ 状态栏、dialogue well 和输入框同宽 | 时间区状态 `TUNING`；DJ 状态 `THINKING`；导航选中 Radio | `PREPARING SESSION`、`TUNING YOUR STATION...`、`400 × 56px` 中性色低速波形、560px 队列骨架、用户原始输入、`Tuning your station...`、固定禁用输入框内显示 `Generating...`；输入框下方不显示步骤摘要 |
| 07 | Detail Sheet：DJ 串讲 | `x 0 / y 0 / w 960 / h 1600` 全屏沉浸节目界面；浅色节目面内 `x 80 / w 800` | `● SPEAKING NOW`；无底部导航 | 顶部 `84 × 6px` 拖动条、`56 × 56px` 关闭按钮、全宽声波区、纯白节目面、节目标题、`If · Bread`、歌曲进度、`800 × 560px` 串讲词卡片、节目整体进度、单一播放/暂停 |
| 08 | Detail Sheet：歌词跟随 | 与 07 共用同一套全屏固定骨架 | `● PLAYING`；无底部导航 | 全宽声波区、纯白节目面、节目标题、`Space Song · Beach House`、歌曲进度、`800 × 560px` 歌词卡片、当前歌词高亮、节目整体进度、单一播放/暂停 |
| 09 | Library 音乐库 | `800px` 单列管理页 | 导航选中 Library | 标题“音乐库”、副标题、本地音乐数量、搜索框、5 条搜索结果、试听图标、加入候选池按钮、导入网易云歌单卡片、服务状态 `CONNECTED`、已导入来源 |
| 10 | Taste 品味档案 | `800px` 单列管理页 | 导航选中 Taste | 标题“你的音乐品味”、编辑品味按钮、品味概览摘要、极简圆环或线性分布、10–12 个常听风格标签、喜欢的声音、避雷规则、场景偏好、最近反馈 |
| 11 | Taste 编辑状态 | `800px` 单列编辑页 | 二级编辑态，底部固定操作区优先 | 返回箭头、标题“编辑音乐品味”、上次更新时间、风格标签编辑、避雷规则输入行、场景规则卡片、Ghost 按钮“取消”、主按钮“保存品味”、数量限制说明 |
| 12 | Programs 节目历史列表 | `800px` 单列管理页 | 导航选中 Programs；分段控制器 All 选中 | 标题“节目”、搜索图标、本周收听摘要、150–180px 节目卡片、日期时间、收藏图标、节目标题、场景摘要、曲目数量、总时长、封面叠放或歌曲名 |
| 13 | 节目历史详情 | `800px` 单列详情页 | 导航选中 Programs | 返回箭头、收藏和更多、`PROGRAM ARCHIVE`、节目元数据、`YOUR SCENE` 摘要卡片、复用场景、重播串讲、DJ 开场自然段、节目队列、节目反馈摘要 |
| 14 | Settings 服务配置 | `820px` 单列设置页 | `● 3 SERVICES ONLINE`；导航选中 Settings | 标题“设置”、服务状态四行、TTS `DEGRADED` 黄色状态、服务配置输入框、密钥遮蔽、Theme Mode、DJ Language、DJ Voice Style、本地数据、底部固定操作区 |
| 15 | Settings 连接检测结果 | `820px` 单列检测页 | 导航选中 Settings | 标题“服务检测”、摘要 `3 OF 4 SERVICES AVAILABLE`、四张检测结果卡片、TTS 展开修复建议、局部降级说明、主按钮“返回 Radio”、次按钮“修改配置” |
...
2. 内容列宽是否匹配页面类型：Radio `560px`、Profile `720px`、管理页 `800px`、Settings `820px`；Detail Sheet 是否覆盖完整 `960 × 1600px` 画布，并在浅色节目面内使用 `800px` 内容列。
3. 底部胶囊导航是否为 `x 300 / y 1504 / w 360 / h 64`；二级编辑态不与导航重复抢占底部，Detail Sheet 打开后不显示底部导航。
4. Radio 三个主页面状态是否共享 `x 200 / w 560` 内容 rail，播放器、队列、DJ 状态栏、dialogue well 和输入框是否左右对齐。
5. Radio 底部输入框是否固定为 `x 200 / y 1400 / w 560 / h 72`；输入框下方是否没有状态文字、连接状态或步骤摘要。
6. Radio 空状态是否只显示 `LIVE`，播放态才显示 `ON AIR`；生成中使用 `TUNING` / `THINKING`，不可误用绿色成功状态。
7. Radio 三态的 Logo、头像、时间、状态点、Tab 图标和 Radio 选中底是否保持同一尺寸。
8. DJ 对话区是否保留 `560 × 284px` dialogue well 边界，DJ 文案是否为自然段，用户输入是否为右对齐弱气泡。
...
本节约束正式前端开发前的视觉资产制作。任务、状态和证据统一记录在 [`tasks/visual-assets.md`](tasks/visual-assets.md)，资产来源、命名和目录契约见 [`assets/README.md`](assets/README.md)。

### 10.1 当前状态与目标交付

当前已存在 15 张 Dark 高保真原型图，但尚未生成归一化基线、Light 页面、Figma 文件、HTML 视觉原型、组件库或 QA 证据。

目标交付为：

* 15 张 Dark 与 15 张 Light 的 `960 × 1600px` Figma 页面。
* 与 Figma 同源的 HTML/CSS/JS 视觉原型，使用 mock 数据演示关键流程。
* Figma Variables、Components、Variants 与真实图片、图标素材。
* Dark 参考基线、同视口截图、并排图、叠层图、差异记录和开发映射。

该工作流属于开发前视觉设计，不代表 React、Backend、API、Audio Engine 或正式产品已经实现。

### 10.2 混合裁决规则

设计资产不得对原型图或文档采用单一来源无条件优先，而应按 Concern 裁决：

1. 已确认原型图决定整体构图、视觉比例、密度、明暗层级和最终观感。
2. 本文件决定 token、公共组件、主题、动效和无障碍一致性。
3. `docs/prd.md` 决定文案、字段、状态语义与产品验收。
4. `docs/user-flow.md` 决定操作路径与异常分支。
5. `prompt.md` 只作为原型生成背景，不覆盖已确认原型和上述权威规则。

生图造成的尺寸漂移、错字、椭圆头像、非等比封面、随机图标或单页样式不得直接固化为组件规范。若按规范修正会明显改变已确认原型观感，相关任务必须进入 `Review`，等待用户确认后继续。

### 10.3 参考图归一化

`design/references/` 下 15 张原始图片实际为 `971 × 1619px`，原文件必须保留不变。设计资产工作流使用高质量重采样生成 `960 × 1600px` Dark 基线，并记录原始尺寸、目标尺寸、文件摘要和处理方式。

归一化只统一画布尺寸，不同时修图、改文案或重排版。原型问题应单独登记并按混合裁决规则处理。

### 10.4 Dark 与 Light 交付

Dark 页面以归一化参考图为视觉基线；固定 slot 和组件边缘误差不得超过 `4px`。

Light 页面没有独立参考图，应满足：

* 与对应 Dark 页面共用 Auto Layout、组件实例、内容和信息层级。
* 只通过 Light Variables 和本文件明确的主题专属材质规则改变表现。
* 单独校准表面层级、边框、文字和状态颜色对比度。
* 不声称对不存在的 Light 参考图进行像素级还原。

### 10.5 Figma 与 HTML 同源要求

每项组件或页面任务必须同时产出可编辑 Figma 结构和对应 HTML 实现。

Figma 必须使用 Variables、Auto Layout、Component Instances 和 Variants，禁止把整页截图作为页面主体。HTML 只承担视觉渲染、状态展示和关键流程验证，不连接 API，不实现持久化、真实音频或正式业务状态管理。

两种资产使用相同 screen ID、主题值、token、组件命名和页面文案。任何一侧的修正都必须同步检查另一侧，禁止形成两套独立设计系统。

### 10.6 逐任务验收

每个页面必须在 `960 × 1600px` 视口提供：
...
最终进入正式前端开发前，30 张页面、3 组关键流程、组件状态、键盘、Focus、44 × 44px 命中区、200% zoom 与 Reduce Motion 检查必须全部通过。
