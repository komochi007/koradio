...
DJ 对话区是 Profile 级持续对话与节目叙事的统一 transcript。它保留编辑感强的电台版式，不采用社交聊天应用的视觉语言；闲聊、澄清、单曲卡片、节目生成进度、DJ 串讲与来源都在同一内部滚动区域连续呈现。
...
* 歧义需求显示简短澄清问题，不启动音乐任务；单曲推荐显示曲名、艺人、推荐理由与 `PLAY NOW` / `PLAY NEXT`；节目生成状态作为 transcript 状态行展示，旧播放器与输入保持可用。
...
当前句或当前歌词行使用浅色面主文本 `#14161A`；当前句中已播放字符继续保持主文本颜色，当前字符使用更强字重，并在字符/词的字体内部按播放进度从待播色平滑渐变到主文本色，避免整字瞬间跳变；未播放字符使用中灰 `#767A80`；句子结束后整句变为已读颜色 `#898A86`，在 `#F5F3F6` 大号正文卡片上保持 `3.15:1` 对比度。避免使用大面积绿色文字高亮，可使用非常轻的词级绿色柔光或小状态点辅助定位，不使用绿色侧边条。

若歌词或 DJ 串讲没有真实逐字时间戳，逐字进度按当前行的可读文字长度在该行时段内估算：中文按单个汉字推进，英文按单词推进，标点与空白跟随前一个单位。该估算只影响高亮进度，不改变 canonical timeline、播放位置或文本内容。

歌词态普通、已读、待播和静态歌词使用 `clamp(18px, 2.7vw, 24px)`、`1.5` 行高；当前歌词使用 `clamp(24px, 3.5vw, 32px)`、`1.32` 行高并保留高字重。歌词卡片上下保留不少于 `32px` 的安全内距，当前行变化后只滚动歌词卡片自身到可视区域中央，首行与末行通过等量上下安全间距保持可达，不能推动外层页面。Electron 最小窗口使用 `clamp(18px, 3.6vw, 20px)` 普通歌词与 `clamp(22px, 4.4vw, 25px)` 当前歌词，顶部歌曲信息区进一步收紧。
