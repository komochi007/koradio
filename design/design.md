...
Electron 下 Detail Sheet 使用实际 viewport，不继承 Radio、Library 等页面的 `960 × 1600px` 内容缩放。波形区与浅色节目面按约 `25% / 75%` 的窗口高度分配，波形柱在节目面上沿下方继续延伸约 `24px`，由上层节目面自然裁切中间部分并填充两侧圆角；节目面始终贴合窗口底部。最小 `430 × 652px` 窗口下，节目标题字号不小于约 `30px`，普通歌词不小于约 `20px`，当前歌词不小于约 `26px`。
...
* 串讲词/歌词卡片：`x 56 / y 700 / w 848 / h 700`，圆角 `32px`；保留 `40px` 内边距；07 串讲词从顶部开始排列，分句组间距 `24px`；08 歌词也从顶部开始排列，内部可滚动但不显示滚动条
...
歌词态普通、已读、待播和静态歌词使用 `clamp(24px, 3.75vw, 36px)`、`1.55` 行高；当前歌词使用 `clamp(34px, 5vw, 48px)`、`1.35` 行高并保留高字重。歌词卡片上下保留不少于 `32px` 的安全内距，当前行变化后只滚动歌词卡片自身到可视区域中央，首行与末行通过等量上下安全间距保持可达，不能推动外层页面。以上字号不应用于 07 DJ 串讲态；Electron 最小窗口使用独立的紧凑字号下限。
...
| 07 | Detail Sheet：DJ 串讲 | `x 0 / y 0 / w 960 / h 1600` 全屏沉浸节目界面；浅色节目面内使用 Detail 专属 `x 56 / w 848` 内容列 | `● SPEAKING NOW`；无底部导航 | 顶部 `84 × 6px` 拖动条、`64 × 64px` 关闭按钮、64 柱白灰平滑包络声波、880ms 传播式波浪、纯白节目面、60px 节目标题、`If · Bread`、歌曲进度、顶部对齐且组间距 24px 的 `848 × 700px` 串讲词卡片、非规则节目整体进度、单一播放/暂停 |
| 08 | Detail Sheet：歌词跟随 | 与 07 共用同一套全屏固定骨架 | `● PLAYING`；无底部导航 | 64 柱白灰平滑包络声波、880ms 传播式波浪、纯白节目面、60px 节目标题、`Space Song · Beach House`、歌曲进度、`848 × 700px` 歌词卡片、当前歌词高亮、非规则节目整体进度、单一播放/暂停 |
