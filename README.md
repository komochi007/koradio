...
> Status: **S1 engineering scaffold complete · S2 platform, data and security foundations complete · S3 backend domain work next · Mock mode only**
> Audience: AI Coding Agents、开发者、维护者  
> Runtime: 当前仓库已有可安装、可开发启动、可生产构建的 Web/Local Service 最小骨架，以及 SQLite/Drizzle migration、首次数据目录 bootstrap、DeviceSettings、ProfilePreferences、脱敏 Health、可回滚数据目录迁移、macOS Keychain Secret Store、受控 File Store、结构化脱敏日志和本地 Session/Origin 防护；仍只使用 Mock Provider 且没有产品 UI、Profile CRUD、节目、播放或真实 Provider
...
- [x] Fastify Local Service health/session/events、DeviceSettings、ProfilePreferences 与数据目录迁移路由已实现；Profiles、Program、播放等产品业务模块尚未实现
...
- [x] SQLite/Drizzle 平台底座已实现：首次启动选择 OS 应用数据目录，版本化 migration、WAL、foreign keys、严格文件权限和失败回滚测试已验证；DeviceSettings、ProfilePreferences 与迁移 job owner 表已落地，其他业务 owner schema 尚未实现
...
当前可以在本地和 GitHub Actions 验证运行版本、workspace、锁文件、frozen install、`check`、三浏览器 E2E、axe、视觉基线，以及 Mock skeleton 的开发/生产启动、认证 REST health、认证后 WebSocket 事件、DeviceSettings/ProfilePreferences 隔离、脱敏服务快照、同源静态托管、SQLite 平台 bootstrap、数据目录迁移成功/回滚/重复请求、受控 File Store、脱敏日志和内存 Session/Origin 安全矩阵。macOS 登录会话还可验证真实 Keychain 往返；这些证据只证明工程、配置、数据、本地 I/O 与 loopback HTTP 安全底座连通，不证明 Profile CRUD、Program、播放、完整业务数据模型或真实 Provider 行为；真实 Provider adapter、TTS helper、其余业务模块和安装包仍不存在。
...
> `apps/*` 与 `packages/*` 边界 manifest 已存在，`apps/server/src/platform/{db,events,files,logging,secrets}`、`modules/device-settings/` 与 `modules/profile-preferences/` 已落地；以下其余业务 module、Web feature、Audio Engine 与 native helper 结构仍来自 `architecture.md`，尚不存在。
...
- 已有 OS 数据目录 bootstrap、SQLite connection、Drizzle migration runner、DeviceSettings/ProfilePreferences/migration job owner 表与原子 bootstrap 指针；Profile、Program 等其余业务 owner schema 尚不存在。
...
- 已有完整 v1 wire contracts；health/session/events、DeviceSettings、ProfilePreferences 和数据目录迁移已有 route/use case，其余业务 contract 尚未实现。
...
S1 工程脚手架和 S2 平台、数据与安全阶段门已关闭。`S2-01`～`S2-05` 的公共 Contracts、SQLite/Drizzle、本地 I/O、HTTP 安全、设备/Profile 配置、脱敏 Health 与数据目录迁移底座均已通过验证。下一关键任务是 `S3-01`：

- 实现 Profiles CRUD、受控头像与 Profile context。
- 让 ProfilePreferences 与默认 TasteOverrides 随 Profile 建立并保持隔离。
- 验证显式 `profileId`、上传安全和 Profile 切换协调边界。
