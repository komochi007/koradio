...
| 已通过阶段门 | 尚无完整阶段门；`S0-01`～`S0-05`、`S0-07`、`S0-08` 已完成；`S0-06` 阻塞 | 解除 S0-06 后关闭 S0，不提前启动 S1 |
| 当前关键路径 | `S0-06 阻塞解除 → S1` | 先完成网易云播放 URL 授权或替代裁决，再启动脚手架 |
...
| 系统架构 | 已完成目标架构，工具链、运行拓扑与包装交付边界已冻结；Provider 可行性仍受网易云播放 URL 授权阻塞 | `architecture.md`、`AI_RULES.md`、`docs/adr/0001-toolchain-and-quality.md`、`docs/adr/0002-runtime-topology.md`、`docs/adr/0003-macos-packaging.md`、`docs/adr/0004-provider-feasibility.md` | 解除 Provider 授权阻塞后进入脚手架 |
...
| S0-06 | 阻塞 | `docs/adr/0004-provider-feasibility.md` 及脱敏验证报告 | Codex、网易云搜索/歌词/歌单、TTS 已验证；网易云播放 URL 端点存在但当前应用未授权 |
...
| Provider 可用性或授权不确定 | 核心生成、搜歌或公开发布可能受阻 | S0 可行性验证、Port 隔离、Mock 和稳定降级；当前需先解除网易云播放 URL 授权 | S0 / S7 |
