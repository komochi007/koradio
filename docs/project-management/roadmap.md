...
| 当前关键路径 | `S6-05` | 通过内部全质量门，形成进入包装阶段的质量基线 |
...
Koradio 已完成开发前产品和视觉定义、可安装/启动/测试/构建的 S1 Mock skeleton、S2 平台/数据/安全底座、S3 后端闭环、S4 P0 前端体验，以及 S5 Library、Taste、Programs 和全量功能验收。S6-01～S6-04 已分别关闭跨层失败、数据生命周期、安全/依赖审计与性能/无障碍回归；bundled native TTS helper 与真实 Provider 产品运行组合尚未完成，S6-05 仍待执行。
...
| S6-05 | 计划 | 内部全质量门报告、缺口清单和可追溯 CI 证据 | 汇总现有质量入口，不提前开始包装或外部 Beta |
