...
| 已通过阶段门 | 尚无完整阶段门；仅 `S0-01` 已完成 | 从 `S0-02` 开始按依赖推进 |
| 当前关键路径 | `S0-02 → S0-03 → S0-04/S0-05 → S1` | 不并行启动依赖尚未关闭的脚手架任务 |
...
| 系统架构 | 已完成目标架构 | `architecture.md`、`AI_RULES.md` | 通过 ADR 关闭工具链、端口和包装形态决策 |
...
当前工作树包含用户尚未提交的文档和视觉语义校准改动。S0 必须先确认开发基线，后续任务不得覆盖或回退这些资产。

## 2. v1.0 发布边界

v1.0 包含 PRD 定义的六项 P0 和三项 P1 能力，并以 macOS 签名、公证安装包直接发布。Windows、Mac App Store、云同步、远程访问、支付、公开社区、自动更新和多音乐源不进入 v1.0。

包装形态仍是受控决策项：S0 通过短 PoC 对比“桌面壳一体包”和“PWA + Local Service 安装器”，裁决后先更新 ADR 与架构，再进入 S7 实现。
...
|---|---|---|---|
| S0-01 | 本次创建 | `docs/project-management/README.md`、`roadmap.md`、`tasks.md`、`release-checklist.md` | 项目管理事实源，不承载产品规则 |
| S0-02 | 计划 | `docs/adr/`、`docs/adr/README.md`、`docs/adr/template.md` | 记录不可逆或高影响工程决策 |
| S0-03～S0-06 | 计划 | `docs/adr/0001-*.md` 起 | 工具链、运行拓扑、包装形态和 Provider 可行性决策 |
...
| macOS 包装形态未定 | 影响运行拓扑、凭据库和发布成本 | 两方案短 PoC、ADR 和干净机器验证 | S0 |
...
| 当前工作树未提交 | 新任务可能覆盖用户资产 | S0 先确认基线，所有改动做最小补丁 | S0 |
