...
| 已通过阶段门 | 尚无完整阶段门；`S0-01`～`S0-05`、`S0-07` 已完成，`S0-06` 正在验证网易云当前 App 权限与 Browser 资源行为 | 完成播放地址 API 权限、CORS/Range 与失败边界探针后验收 S0-06；不提前启动 S1 |
| 当前关键路径 | `S0-06（网易云官方播放 URL 真实探针）→ S1` | Browser Audio Engine 已确认；CLI 只用于已验证查询，Local Service 使用官方 URL API 的可行性通过后才能裁决 |
...
| S0-06 | 已创建 · 提议 · 进行中 | `docs/adr/0004-provider-feasibility.md`、`docs/adr/evidence/0004-provider-feasibility-validation.md` | 官方播放 URL 接口已确认；等待当前 App 权限、真实 URL、CORS/Range、过期与不可播探针 |
...
| Provider 可用性或授权不确定 | 核心生成、搜歌或公开发布可能受阻 | S0 已排除非官方路径、验证官方查询并确认播放 URL 契约；当前 App 权限或 Browser 资源探针失败时替换 Provider，公开授权仍由 S7 关闭 | S0 / S7 |
