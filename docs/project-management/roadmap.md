...
| 已通过阶段门 | 尚无完整阶段门；`S0-01`、`S0-02`、`S0-03`、`S0-07` 已完成 | 继续关闭 S0 剩余决策，不提前启动 S1 |
| 当前关键路径 | `S0-04 → S0-05 → S1`；`S0-06` 可独立推进但同属 S0 阶段门 | 不并行启动依赖尚未关闭的脚手架任务 |
...
| 系统架构 | 已完成目标架构，工具链基线已冻结 | `architecture.md`、`AI_RULES.md`、`docs/adr/0001-toolchain-and-quality.md` | 通过 ADR 关闭运行拓扑、端口和包装形态决策 |
...
| S0-04～S0-06 | 计划 | `docs/adr/0002-*.md` 起 | 运行拓扑、包装形态和 Provider 可行性决策 |
