...
| 已通过阶段门 | 尚无完整阶段门；`S0-01`～`S0-05`、`S0-07` 已完成，`S0-06` 因播放 owner 决策阻塞 | 裁决网易云 Browser 资源、外部客户端播放或替换 Provider 后验收 S0-06；不提前启动 S1 |
| 当前关键路径 | `S0-06（网易云播放 owner 裁决）→ S1` | Apple 系统 TTS 与网易云查询链路已通过；CLI 不返回 Browser Audio Engine 播放资源，需先裁决公共播放行为 |
...
| S0-06 | 已创建 · 提议 · 阻塞 | `docs/adr/0004-provider-feasibility.md`、`docs/adr/evidence/0004-provider-feasibility-validation.md` | 三类 Provider 核心探针已完成；网易云 CLI 不返回 Browser 播放资源，等待播放 owner 裁决 |
...
| Provider 可用性或授权不确定 | 核心生成、搜歌或公开发布可能受阻 | S0 已排除非官方网易云路径并验证官方查询能力；Browser 播放资源、播放 owner 与公开授权未裁决前保持阻塞 | S0 / S7 |
