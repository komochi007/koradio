...
| 当前关键路径 | `S4-01` | 重构 Web App Shell、路由与共享 transport |
| 产品实现状态 | S3 后端闭环完成；无 P0 产品 UI 或浏览器播放闭环 | 不把连接探针、Mock 后端或设计 fixture 描述为 Radio、Audio Engine 或可用产品 UI |
...
Koradio 已完成开发前产品和视觉定义、可安装/启动/测试/构建的 S1 Mock skeleton、S2 平台/数据/安全底座，以及 S3 Profiles、Library、Feedback/Taste、Programs/Playback、生成 Job、Provider adapters 与确定性 Mock 后端闭环。当前 App Shell 仍只是连接探针，产品 UI、Browser Audio Engine、bundled native TTS helper 与真实 Provider 产品运行组合尚未实现。

| 进度维度 | 当前状态 | 证据 | 下一门槛 |
|---|---|---|---|
| 产品定义 | 已完成开发前校准 | `docs/prd.md`、`docs/user-flow.md` | 实现中发现冲突时先回写权威文档 |
| 系统架构 | 目标架构已完成；S1 Web/Server/Contracts/Tokens、S2 平台边界与 S3 八个后端模块、Provider adapters、生成 Job、事务和事件已实装 | `architecture.md`、`AI_RULES.md`、ADR 0001～0004、S1/S2/S3 测试与 S3-07 验收记录 | S4 Web App Shell 与 P0 前端组合 |
...
| 产品功能 | 后端闭环已形成；产品交互闭环未形成 | Profile CRUD、配置、Library、Taste/Feedback、Program/Playback、生成 Job、Provider adapters 与 Mock 闭环已实现；Radio UI 与 Browser Audio Engine 未实现 | S4 P0 与 S5 P1 功能阶段门 |
