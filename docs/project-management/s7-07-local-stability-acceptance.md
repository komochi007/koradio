...
| 2026-08-18 | 0 | 0 | 0 | 更新器缓存依赖强制刷新、真实候选构建与原位替换复验 | 无 | S7-09-006 | 待本机复验；不计入真实生成或播放样本 |
...
| S7-09-006 | 2026-08-17 | Medium | 已安装应用的独立 updater cache 在候选包构建末段缺少 optional native dependency，导致自动更新失败并拒绝替换旧版。 | 可信远端更新应能完成候选构建与原位替换；实际固定入口安全保持旧版，未发生替换。 | 候选构建前，使用应用随附 Corepack 的 pnpm 以 `--frozen-lockfile --force` 刷新更新器自有 checkout 的锁定依赖，再执行既有构建、验证与原位替换；不删除用户目录或缓存以外的内容。 | 新增更新器依赖刷新顺序回归；待完成真实更新、包验证与 strict codesign 复验后收口。 |
