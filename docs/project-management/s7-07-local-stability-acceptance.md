...
| S7-09-007 | 2026-08-21 | High | 直接以带 Hardened Runtime 的包内 Node 手工运行更新器时，候选构建无法加载缓存原生模块；正常应用启动须使用临时 ad-hoc 更新 Node。 | 自动更新必须经 Electron 的更新前置链路使用临时 ad-hoc Node，继续保持 lockfile、候选包验证与原位替换边界。 | 更新器缓存安装保留 `--frozen-lockfile` 并强制重建 optional dependency；只删除 `~/Library/Caches/Koradio/Updater/source/node_modules`，不触及工作区、模型、用户数据或应用备份。 | `macos-update.test.ts` 7 项通过；从 `/Applications/Koradio.app` 正常启动后，以临时 ad-hoc Node 成功构建、严格校验并原位替换为 `0.0.200`（`e0cace7`），新 Local Service 与 Electron Renderer 均已启动。 |
