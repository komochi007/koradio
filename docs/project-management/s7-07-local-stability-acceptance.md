...
| S7-09-002 | 2026-08-05 | High | Electron 更新器在缓存 checkout 构建新包时，`build-macos.mjs` 会先静态解析 `@electron/packager`，而依赖安装尚未执行。 | 更新器应先完成 frozen install，再加载 Electron 打包器；实际以 `ERR_MODULE_NOT_FOUND` 终止且不打开产品。 | 移除顶层静态 import，改在 `pnpm install --frozen-lockfile` 成功后动态加载打包器。 | `macos-update.test.ts` 断言依赖安装顺序；待实际更新器完成构建、替换和启动后补充 package/运行证据。 |
