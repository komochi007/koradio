...
| S7-09-007 | 2026-08-21 | High | 更新器已经删除并按锁文件重建自身依赖，但候选构建仍缺少 rolldown 的 arm64 optional binding，安全地保持旧 app 而无法替换。 | 受控缓存重建应恢复所有当前平台所需的 optional binding，并继续保持 lockfile、候选包验证与原位替换边界。 | 更新器的缓存安装保留 `--frozen-lockfile`，同时使用 pnpm `--force` 重建 optional dependency；只删除 `~/Library/Caches/Koradio/Updater/source/node_modules`，不触及工作区、模型、用户数据或应用备份。 | `macos-update.test.ts` 断言强制冻结安装先于候选构建；更新器成功构建、验证并替换当前 `/Applications/Koradio.app` 后补充版本与 Live 验收事实。 |
