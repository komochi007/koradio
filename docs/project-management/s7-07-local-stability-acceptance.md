...
| 2026-07-24 | 0 | 0 | 0 | 桌面 PWA 固定原型画布与尺寸提示回归 | 无 | S7-07-004 | 修复并待继续真实试用 |
...
| S7-07-004 | 2026-07-24 | Medium | 桌面 PWA 会按窗口自适应放大内容并允许一级内容区滚动，与 `960 × 1600px` 原型固定画布不一致。 | 桌面应以原尺寸居中的固定画布呈现且外层无滚动；实际在较矮桌面窗口中内容被放大并出现滚动。 | 已加入精细指针桌面画布门控：宽度达到 `960px` 时固定为 `960 × 1600px`，高度不足时显示尺寸提示；长内容只在隐藏滚动条的内部区域滚动。 | `desktop-canvas.component.test.tsx` 覆盖三种模式与窗口恢复；Chromium Radio E2E 验证尺寸提示、固定画布、无外层滚动；完整 `pnpm check` 通过。 |
