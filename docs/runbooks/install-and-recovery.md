...
- Google Chrome 安装在 `/Applications/Google Chrome.app`；launcher 只用 `--app=<loopback URL>` 打开独立窗口，不安装 Chrome PWA。
...
2. launcher 使用包内 Node 和 updater，在独立缓存 checkout 中抓取 `https://github.com/komochi007/koradio.git` 的 `origin/main`；不得读取或修改日常开发工作树。
...
2. 安装前退出菜单栏中的 Koradio，并确认 Chrome Apps、其他 `Applications` 目录与 Dock 中不存在第二个 Koradio PWA 或 `.app`。
...
5. 如果确需离线诊断旧包，只能直接运行备份中的 launcher `--smoke`，不得把备份改回 `.app` 或添加到 Launchpad。
...
| 唯一入口 | 只有固定 `/Applications/Koradio.app` 使用 `.app` 后缀；Chrome PWA shim 和其他 Koradio `.app` 不存在。 |
| 启动前更新 | 每次正常打开都读取可信 `origin/main`；更新失败不启动 Local Service 或 Chrome 产品窗口。 |
