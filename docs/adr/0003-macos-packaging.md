...
- 每个 DMG 包含一个薄原生 launcher `.app`、对应架构的 Node 24.18.0 精简 runtime、`pnpm --prod deploy` 生成的 Server 文件树、built PWA assets，以及由 [ADR 0004](0004-provider-feasibility.md) 增加的同架构 Apple TTS native helper；helper 不扩张 launcher 的生命周期职责。
...
1. 用 `Developer ID Application` 对 bundled Node、Apple TTS native helper 和 launcher bundle 逐层签名，启用 Hardened Runtime、安全时间戳，禁止 `get-task-allow`。
...
| 未来公开发布 | launcher、bundled Node 与 Apple TTS helper 的 Developer ID、notary ticket、Gatekeeper 与独立干净用户未验证；外部分发保持禁止 | S7 发布硬门 |
