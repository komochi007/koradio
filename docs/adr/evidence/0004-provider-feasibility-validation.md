...
1. 在[网易云官方开放平台](https://developer.music.163.com/st/developer/apply/account?type=INDIVIDUAL)创建或指定测试应用；在受控本机运行交互式 `ncm-cli configure`，避免把 AppID/私钥写入命令参数或 shell history，再运行 `ncm-cli login` 并用网易云音乐 App 扫描终端二维码；记录应用能力和审批状态的脱敏截图或文字证据。
