...
本任务已经证明 Codex 的本地非交互调用链可用，也确认网易云存在官方开放平台与个人开发者 CLI、Fish Audio 存在官方 TTS API。但当前环境缺少已授权的网易云 App 凭据和用户登录态，无法验证搜索、歌词与浏览器可播资源；Fish Audio API 端点还在 TLS 握手阶段被当前网络阻断。因此本 ADR 只能记录推荐方案和解阻条件，不能作为已接受的实现依据。完整探针结果见[脱敏验证报告](evidence/0004-provider-feasibility-validation.md)。

## 2. 决策范围

### 包含

- Codex、网易云和可选 Fish Audio TTS 的推荐接入形态、能力边界与版本探针。
...
- 做法：Codex 使用本机 `codex exec`；网易云使用官方开放平台批准的 API、SDK 或个人开发者 CLI；TTS 使用 Fish Audio 官方 REST API。所有结果先在服务端校验并归一化。
- 收益：来源、鉴权和发布责任可追踪；不依赖逆向协议；可用 Mock 隔离 CI 与付费服务。
- 代价：需要用户自行完成 Provider 注册、凭据配置和登录；网易云个人预览与公开分发可能需要不同的批准形态。
- 风险：网易云官方个人 CLI 是否能稳定提供浏览器可播资源和歌词仍未证实；其动态命令树依赖有效凭据和登录态。
- 验证结果：Codex 已通过最小真实调用；网易云和 Fish Audio 的文档、无凭据与失败路径已采样，但成功调用证据不完整。
...
本 ADR 当前推荐方案 A，但尚未生效。只有以下条件全部满足并由项目所有者确认后，状态才能改为 `已接受`：

1. 使用项目所有者在网易云官方开放平台创建的受控测试应用和本机登录态，证明歌曲搜索、可见性/可播性判断、歌词、歌单导入，以及可交给 Browser Audio Engine 的播放资源或等价受控播放方式。
2. 使用受控 Fish Audio 测试凭据和可达网络完成成功、无凭据、错误凭据、超时与响应媒体类型采样；如果继续不可达，只能把 TTS 明确保持为关闭并验证文字降级。
3. 先在 `docs/prd.md` 与 `architecture.md` 裁决 `neteaseApiUrl` 和官方 CLI/API 路径的冲突，再同步相关摘要；不得让 Adapter 实现反向定义公共行为。
...
| Fish Audio | 官方 `POST /v1/tts`；Bearer 只从 Secret Store 注入；固定接受的音频媒体类型并记录脱敏模型版本 | 单次 45 秒，且不得突破生成总预算；取消 HTTP 请求并丢弃迟到音频 | 401、402、422 不重试；429 仅按 `Retry-After` 重试 1 次；5xx/瞬态网络错误最多 1 次 | 校验状态、媒体类型和大小后写入 File Store；任何失败都退回文字稿，不影响节目生成 |
...
- PRD、用户流程、视觉规则、公共 contract 和数据模型均未被本提议修改。
...
| Fish Audio | 合法短音频与媒体类型 | 401、402、422、429、5xx、错误媒体类型、超大响应、超时 | Secret 不回显、有限重试、File Store 边界和文字降级 |

fixture 只表达 Koradio Adapter 的输入边界和归一化输出，不复制整份真实 Provider 响应，也不成为 `/api/v1` contract 的事实源。

## 8. 发布风险结论

| 使用范围 | Codex | NetEase | Fish Audio |
|---|---|---|---|
| Personal Local Preview | 用户自有登录或 API key；不随应用分发 | 仅允许用户自有官方个人开发者应用、官方 CLI 与登录态；禁止兼容/逆向 API | 免费层只按个人、非商业边界使用；默认关闭仍可完整降级 |
| 未来公开分发 | 发布前复核当时的 Codex 产品条款、认证方式和配额；不得内置共享凭据 | 必须取得适合公开应用的官方批准、应用验收和内容权利；个人 CLI 探针不能作为公开授权证据 | 商业发布必须使用允许商业用途的付费服务或书面许可，并披露 AI 生成语音、确认声音权利 |
...
| 自动检查 | Codex 四类探针、网易云 CLI 版本和失败路径探针已执行；文档链接、状态一致性和 diff 检查由本 PR 验证 | [验证报告](evidence/0004-provider-feasibility-validation.md) |
| 人工/外部验证 | 网易云成功链路和 Fish Audio 真实调用待项目所有者在本机安全配置后执行 | [验证报告解阻清单](evidence/0004-provider-feasibility-validation.md#8-解阻与复验清单) |
...
| 文档 | 是否需要修改 | 原因或结果 |
|---|---|---|
| `docs/prd.md` | 接受前需要 | 裁决 `neteaseApiUrl` 与官方 CLI/API 的设备设置及播放行为 |
| `docs/user-flow.md` | 待复验后判断 | 若官方登录或播放形态改变用户步骤，必须先同步流程 |
| `architecture.md` | 接受前需要 | 固定 MusicProvider Adapter、进程调用和播放资源边界 |
| `design/design.md` | 当前不需要 | 本任务不改变 UI 或视觉规则 |
| `AI_RULES.md` | 待接受时判断 | 仅当需新增不可违反的 Provider 进程/协议规则时更新 |
| `README.md` / `context.md` | 当前不需要 | 提议不能写成稳定事实；接受后再同步当前状态与领域摘要 |

## 11. 后续任务

- 复验并接受本 ADR：完成网易云与 Fish Audio 的受控真实调用，取得项目所有者对推荐方案的确认，并同步 PRD/架构冲突。
