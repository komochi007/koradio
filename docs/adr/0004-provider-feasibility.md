...
1. 项目所有者明确选择并验证以下之一：取得网易云官方、合法且可交给 Browser Audio Engine 的播放资源能力；或接受网易云客户端/CLI 播放器成为实际播放 owner；或替换音乐 Provider。
2. 先在 `docs/prd.md`、`docs/user-flow.md` 与 `architecture.md` 裁决播放 owner，并同步 `neteaseApiUrl` 和官方 CLI/API 配置冲突；不得让 Adapter 实现反向定义公共行为。
...
| NetEase | 仅使用官方开放平台批准通道；个人预览可调用用户安装并配置的固定版本 `ncm-cli`，启动时检查版本、登录态和必需动态命令 | 健康检查不超过 PRD 15 秒；搜索/资源请求必须可取消并忽略迟到结果 | 鉴权、未登录、命令缺失、不可见/不可播不重试；瞬态网络失败最多重试 1 次；PRD 的换关键词重试属于业务流程 | 映射为内部音乐模型；同时尊重 `visible`/`playFlag`；缺歌词可继续，核心资源不可用则阻塞该曲目；CLI 不返回 Browser 资源，播放 owner 裁决前禁止实施；禁止非官方 fallback |
...
- 网易云官方查询能力已验证，但 Browser Audio Engine 与外部客户端播放二选一会改变公共行为、状态事实源和实现范围，不能由本任务静默决定。
...
| 人工/外部验证 | 只剩网易云 Browser 播放资源能力或外部播放器产品边界待项目所有者裁决并验证 | [验证报告解阻清单](evidence/0004-provider-feasibility-validation.md#8-解阻与复验清单) |
...
- 复验并接受本 ADR：取得项目所有者对网易云播放 owner 和官方 MusicProvider 形态的确认，完成对应资源验证并同步 PRD/流程/架构冲突。
