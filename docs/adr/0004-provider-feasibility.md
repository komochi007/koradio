...
本任务已经证明 Codex 的本地非交互调用链可用。项目所有者于 2026-07-15 决定 v1 不使用 Fish Audio，改用 Apple 系统 `AVSpeechSynthesizer`；该本机语音链路、失败处理和双架构 deployment target 已通过探针。网易云官方 CLI 也已在受控本机完成配置与登录，歌曲搜索、混合可播性、歌词、歌单导入和进程超时均有成功证据；但 CLI 只提供本机播放器/网易云客户端控制，没有返回现有 Browser Audio Engine 所需的播放资源。项目所有者已明确 v1 保持 Browser Audio Engine，拒绝把外部播放器作为播放 owner；当前只剩网易云官方 Browser 播放资源能力与授权未被证明，因此本 ADR 仍不能作为已接受的完整 Provider 实现依据。完整探针结果见[脱敏验证报告](evidence/0004-provider-feasibility-validation.md)。
...
- 风险：网易云官方个人 CLI 的查询能力依赖有效凭据、动态命令树和登录态，并且没有对外返回浏览器可播资源。
- 验证结果：Codex 与 Apple 系统 TTS 已通过最小真实调用、错误边界和超时/取消探针；网易云已通过登录、搜索、可播性、歌词、歌单与进程超时探针，但只证明外部播放器控制，不满足现有 Browser Audio Engine contract。
...
1. 取得网易云官方、合法且可交给 Browser Audio Engine 的播放资源能力，并验证资源成功、过期、不可播和取消；若官方能力不可获得，则发起音乐 Provider 替换决策，禁止回退到外部播放器或非官方兼容 API。
2. 在已确认的官方 API/SDK 能力基础上同步 `neteaseApiUrl` 与 CLI/API 配置边界；CLI 只能承担已验证的查询能力，不得成为播放 owner，也不得让 Adapter 实现反向定义公共行为。
...
| NetEase | 仅使用官方开放平台批准通道；个人预览可调用用户安装并配置的固定版本 `ncm-cli` 做查询，启动时检查版本、登录态和必需动态命令 | 健康检查不超过 PRD 15 秒；搜索/资源请求必须可取消并忽略迟到结果 | 鉴权、未登录、命令缺失、不可见/不可播不重试；瞬态网络失败最多重试 1 次；PRD 的换关键词重试属于业务流程 | 映射为内部音乐模型；同时尊重 `visible`/`playFlag`；缺歌词可继续，核心资源不可用则阻塞该曲目；官方 Browser 资源能力验证前禁止实现真实播放；CLI 不得成为播放 owner；禁止非官方 fallback |
...
- 网易云官方查询能力已验证，但当前个人 CLI 不满足已确认的 Browser Audio Engine 播放 contract；取得额外官方能力或替换 Provider 前不能进入实现。
...
| 自动检查 | Codex 四类探针、Apple TTS 成功/voice/取消/双架构 typecheck、网易云登录/搜索/歌词/歌单/超时与只读播放契约探针已执行；文档检查由本 PR 验证 | [验证报告](evidence/0004-provider-feasibility-validation.md) |
| 人工/外部验证 | Browser Audio Engine 已由项目所有者确认；只剩网易云官方 Browser 播放资源能力与授权待取得并验证 | [验证报告解阻清单](evidence/0004-provider-feasibility-validation.md#8-解阻与复验清单) |
...
| 文档 | 是否需要修改 | 原因或结果 |
|---|---|---|
| `docs/prd.md` | 已部分同步 | Apple TTS 地址/密钥已移除；Browser Audio Engine 保持不变，`neteaseApiUrl` 需在官方 Browser API/SDK 明确后同步 |
| `docs/user-flow.md` | 已同步 TTS | TTS 未配置改为原生 helper/标准语音不可用；Browser 播放流程保持不变 |
| `architecture.md` | 已部分同步 | Apple native helper 与 Browser Audio Engine 边界保持；MusicProvider 配置需在官方 Browser API/SDK 明确后同步 |
...
- 复验并接受本 ADR：取得网易云官方 Browser 播放 API/SDK 能力与授权并完成资源验证；若无法取得，则发起音乐 Provider 替换决策。
