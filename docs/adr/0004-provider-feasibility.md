...
本任务已经证明 Codex 的本地非交互调用链可用。项目所有者于 2026-07-15 决定 v1 不使用 Fish Audio，改用 Apple 系统 `AVSpeechSynthesizer`；该本机语音链路、失败处理和双架构 deployment target 已通过探针。网易云官方 CLI 也已在受控本机完成配置与登录，歌曲搜索、混合可播性、歌词、歌单导入和进程超时均有成功证据；CLI 本身只提供本机播放器/网易云客户端控制，但官方开放平台另有[获取歌曲播放 url](https://developer.music.163.com/st/developer/document?docId=3d2c9f695ff24f4ea37611614b7f7856)接口。项目所有者已明确 v1 保持 Browser Audio Engine，拒绝把外部播放器作为播放 owner；当前剩余缺口收敛为当前 App 的播放地址 API 授权，以及真实 URL 的 CORS、Range、过期和不可播行为。本 ADR 在这些探针完成前仍不能作为已接受的完整 Provider 实现依据。完整结果见[脱敏验证报告](evidence/0004-provider-feasibility-validation.md)。
...
- 风险：网易云官方个人 CLI 的查询能力依赖有效凭据、动态命令树和登录态；播放 URL API 还依赖单独的 API 分组、RSA-SHA256 签名、登录令牌和网易云分配或确认的设备字段。
- 验证结果：Codex 与 Apple 系统 TTS 已通过最小真实调用、错误边界和超时/取消探针；网易云已通过 CLI 登录与查询探针，官方文档也确认播放 URL 契约，但当前 App 授权与 Browser CORS/Range 尚未完成真实验证。
...
本 ADR 当前推荐方案 A。Apple 系统 TTS 与 Browser Audio Engine 子决策已由项目所有者确认；后者保持既有 PRD、用户流程与架构行为，不需要改写权威文档。完整 ADR 只有以下剩余条件全部满足后才能改为 `已接受`：

1. 确认当前 App 已获官方“获取播放地址 API”分组，并验证资源成功、过期、不可播、取消、CORS 与 HTTP Range；若授权或 Browser 资源能力不可获得，则发起音乐 Provider 替换决策，禁止回退到外部播放器或非官方兼容 API。
2. 在真实探针基础上同步 `neteaseApiUrl`、App/设备配置、Secret Store 和 Local Service 签名边界；CLI 只能承担已验证的查询能力，不得成为播放 owner，也不得让 Adapter 实现反向定义公共行为。
...
| NetEase | 仅使用官方开放平台批准通道；CLI 做已验证查询，Local Service 调用 `/openapi/music/basic/song/playurl/get/v2` 并持有签名、令牌与设备参数 | 健康检查不超过 PRD 15 秒；搜索/资源请求必须可取消并忽略迟到结果；播放前按有效期刷新 URL | 鉴权、未登录、命令缺失、`10003`、`10004` 不重试；`10002`/`10005` 与瞬态网络失败在预算内最多重试 1 次 | 同时尊重 `visible`/`playFlag` 与播放 URL `subCode`；`freeTrail` 只允许试听区间；签名 URL 不持久化；真实 CORS/Range 验证前禁止实现真实播放；CLI 不得成为播放 owner；禁止非官方 fallback |
...
- 网易云官方播放 URL 能力已经文档确认，但当前 App 授权与真实 Browser 资源行为未验证；在探针通过或替换 Provider 前不能进入实现。
- 未来公开分发不能沿用个人预览结论，仍需公司开发者能力、应用验收与内容权利复核。
- 官方开放平台路径要求修改设备设置字段和运行时边界，需在真实探针后同步产品与架构。
...
| 自动检查 | Codex 四类探针、Apple TTS 成功/voice/取消/双架构 typecheck、网易云 CLI 登录/查询/超时探针与官方播放 URL 契约检查已执行；文档检查由本 PR 验证 | [验证报告](evidence/0004-provider-feasibility-validation.md) |
| 人工/外部验证 | Browser Audio Engine 已确认；当前 App 的播放地址 API 分组、成功 URL、CORS/Range、过期与不可播仍待验证 | [验证报告解阻清单](evidence/0004-provider-feasibility-validation.md#8-解阻与复验清单) |
...
| 文档 | 是否需要修改 | 原因或结果 |
|---|---|---|
| `docs/prd.md` | 已部分同步 | Apple TTS 地址/密钥已移除；Browser Audio Engine 保持不变，`neteaseApiUrl` 需在当前 App 授权与真实 URL 探针后同步 |
| `docs/user-flow.md` | 已同步 TTS | TTS 未配置改为原生 helper/标准语音不可用；Browser 播放流程保持不变 |
| `architecture.md` | 已部分同步 | Apple native helper 与 Browser Audio Engine 边界保持；MusicProvider 签名、令牌和设备配置需在真实 URL 探针后同步 |
...
- 复验并接受本 ADR：确认当前 App 的播放地址 API 授权并完成 Browser 资源验证；若无法取得，则发起音乐 Provider 替换决策。
