...
| 桌面 standalone PWA | 使用 `max-width: 960px`、`height: 100dvh` 的居中自适应画布，不设置固定宽高比且不得整体缩放；窗口、画布和页面外层不滚动，Radio 的 Queue / DJ 对话、管理页主内容和 Detail 文本卡片按各自边界独立滚动；普通浏览器与手机保留响应式布局 |
| Radio 输入操作 | 语音与发送按钮在同一布局族中必须使用相同的圆角正方形尺寸；Prototype Desktop `64 × 64px`，运行时 standalone PWA 随窗口高度在 `44–56px` 连续调整，Mobile `48 × 48px` |
