...
| 视觉基线一致性 | VDA-17 已冻结 | VDA-17 HTML 主源与 60 张 `960 × 1600px` 正式基线继续作为高保真原型事实源；运行时 desktop standalone PWA 另按内置屏代表 viewport 建立响应式基线 |
...
| 维度 | 规则 |
|------|------|
| 原型画布 | 高保真原型继续以 `960 × 1600px`、`3:5` 竖版桌面应用截图为基准，不展示设备模型、浏览器外框或营销落地页；该尺寸不再作为运行时 PWA 的固定比例约束 |
| 内容列宽 | Prototype 保留 Radio `816px`、Profile `848px`、管理页 `840px`、Settings `832px` 与 Detail `848px` 基线；Desktop standalone PWA 的全部单列运行时内容统一最大 `720px` |
...
| 桌面 standalone PWA | 使用最大 `720px`、`height: 100dvh` 的居中单列画布；launcher 默认外窗 `720 × 840px`，最小支持外窗 `680 × 760px`，任一维度不足时显示窗口过小提示而不继续挤压业务界面；尺寸判断使用外窗尺寸，页面缩放不得误触发提示 |
| Radio 输入操作 | 语音与发送按钮在同一布局族中必须使用相同的圆角正方形尺寸；Prototype Desktop `64 × 64px`，Desktop standalone PWA `44 × 44px`，Mobile `48 × 48px` |
...
| DJ 对话区 | Prototype 保留 `816 × 288px` dialogue well；Desktop standalone PWA 使用有最小高度的独立局部滚动区，Queue 折叠后获得释放空间；DJ 文案和用户弱气泡统一使用 `16px / 24px` |
| Detail Sheet | 覆盖完整产品界面，不露出底层 Radio 页面；串讲与歌词随播放进度逐词高亮，优先使用 Provider/TTS 精确 token 时间，缺失时按行或句时长与文本权重估算并明确标识；文本卡片局部滚动且不显示滚动条 |
| Library 本地音乐 | 不显示“加载更多”；本地音乐放在约 `260–340px` 的卡片内滚动，距底部约 `160px` 自动加载下一页，隐藏滚动条但保留键盘与辅助技术语义 |
...
Desktop standalone PWA 的时间、日期与状态必须按“时间 → 日期 → 状态”纵向居中排列。运行时播放器高度为 `184–196px`，歌曲标题与艺人/专辑信息各占一行并使用省略号，完整内容保留在可访问名称和悬停提示中。页面外层不滚动；Queue、DJ 对话、管理集合和 Detail 文本是明确的局部滚动边界。
...
- 用户播放到 DJ 串讲段落 → 系统将串讲词切为主内容并随播放逐词高亮 → 精确 TTS marker 优先，缺失时按句窗口和文本权重估算。
- 用户播放到歌曲段落 → 系统将歌词切为主内容并逐词高亮 → 增强逐字歌词优先，普通 LRC 在行窗口内估算词时间，纯文本歌词按歌曲总时长估算；当前行自动滚动到卡片中央，歌词卡片保留键盘滚动且不显示可见滚动条。
...
[用户点击状态栏]
  ├─ 当前有节目 → 打开全屏沉浸节目界面
  │      ├─ 播放 DJ 段 → 串讲词为主 + 逐词高亮 + 歌词辅助
  │      ├─ 播放歌曲段 → 歌词为主 + 逐词高亮 + 串讲辅助
  │      ├─ 歌词缺失 → 显示无歌词文案
  │      └─ 用户关闭 → 回到 Radio，播放不中断
  └─ 当前无节目 → 提示先生成电台
```
...
| DJ 跟随 | 当前时间线为 TTS 段 | 时间线进入歌曲段或关闭 | 串讲词作为主内容逐词高亮，当前句自动居中 |
| 歌词跟随 | 当前时间线为歌曲段 | 时间线进入 DJ 段或关闭 | 歌词作为主内容逐词高亮，当前行自动居中 |
...
| TTS 时间戳缺失 | TTS 不返回 marker | 按句时长和文本权重估算逐词高亮 | 显示 `Approximate word timing` |
| LRC 时间轴缺失 | 歌词存在但无逐行时间 | 按歌曲总时长和文本权重估算逐词高亮 | 显示 `Approximate word timing` |
