...
- 用户播放到歌曲段落 → 系统将歌词切为主内容并按 LRC 行高亮 → 用户看到普通歌词以 `clamp(18px, 2.7vw, 24px)` 展示、当前歌词以 `clamp(24px, 3.5vw, 32px)` 高字重展示并随播放自动滚动到卡片中央；当前句中已播放字符保持主文本高亮，当前字符在字体内部随播放进度渐变高亮，不整字瞬间跳变，句子结束后整句恢复已读颜色；歌词卡片上下保留不少于 `32px` 安全间距，支持键盘滚动且不显示可见滚动条，首尾内容均可到达。Electron 最小窗口下 Detail Sheet 使用实际 viewport，并将顶部歌曲信息区收紧，普通歌词使用 `clamp(18px, 3.6vw, 20px)`、当前歌词使用 `clamp(22px, 4.4vw, 25px)`。
