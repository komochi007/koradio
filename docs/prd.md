...
- 用户播放到歌曲段落 → 系统将歌词切为主内容并按 LRC 行高亮 → 用户看到普通歌词以 `clamp(24px, 3.75vw, 36px)` 展示、当前歌词以 `clamp(34px, 5vw, 48px)` 高字重展示并随播放自动滚动到卡片中央；当前句中已播放字符保持主文本高亮，当前字符使用更强字重，句子结束后整句恢复已读颜色；歌词卡片保留键盘滚动且不显示可见滚动条，首尾内容均可到达。Electron 最小窗口下 Detail Sheet 使用实际 viewport，并将标题与歌词设置可读字号下限。
