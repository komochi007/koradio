...
- S1 工程脚手架、S2 平台/数据/安全底座、S3 核心领域与 Provider 后端和 S4 P0 前端阶段门已通过；S4-01～S4-06 App Shell、Onboarding、Profile/Settings、服务检测、安全切换、Radio 三态/节目生成交互、唯一 Audio Engine/多标签租约、Detail Sheet/歌词与串讲跟随及七类反馈 UI，以及 S5-01 Library 搜索/试听/候选池/歌单导入和 S5-02 Taste 查看/人工编辑已实现并验证，下一关键任务是 S5-03 Programs 历史、复用与收藏。
...
- `architecture.md` 中超出 S3 Profiles/Library/Feedback/Taste/Programs/Playback/生成 Job/Provider Adapters、平台安全底座与 S4 P0 前端体验的 P1 产品页面、native helper 和包装运行行为仍是目标设计；当前事实以本节与真实仓库为准。
