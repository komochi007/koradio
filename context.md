...
- **UX-31 已完成并验收**：Daily Mix 顶部右侧使用可复用的确定性 SVG 声场组件，72 条极细黑白灰曲线分成三组共享低频结构并交错起伏，辅以三层低透明曲面和四向渐隐；图形保持静态、无随机重算，在宽窄窗口中通过裁切与占比变化避免横向挤压、边界和闪烁。`PLAY ALL` 图标改为黑色正圆与白色三角；Programs 区块/列表间距统一为 `24px`/`16px`，Daily 卡片使用手型指针；Radio 进度条悬停或键盘聚焦时以 `180ms` 平滑增高并显示 `10px` 绿色拖动控件。macOS `0.0.240` 已通过包装验证与项目所有者验收。
