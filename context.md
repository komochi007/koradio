...
- Codex、网易云通过 Backend Adapter 接入；可选 TTS 由 Backend TTS Port 调用 bundled macOS native helper，再使用 Apple `AVSpeechSynthesizer`。
...
- 网易云官方接入的最终配置字段、播放资源边界，以及数据库与其他业务依赖的具体包和精确版本。
- 尚未创建的 monorepo、manifest、锁文件、配置、CI workflow 与 script 已经可用。

工具链实现必须遵循 ADR 0001；运行拓扑实现必须遵循 ADR 0002；macOS 包装与交付边界必须遵循 ADR 0003，并同步 README、工程配置和权威文档。
