...
- S1 工程脚手架阶段门已通过；S2-01～S2-03 的完整 v1 公共 Contracts、SQLite/Drizzle 数据底座、macOS Keychain Secret Store、受控 File Store 与结构化脱敏日志已实现并验证，下一关键任务是 S2-04 本地 Session、Origin 与事件连接防护。
...
- React/Vite App Shell 与 Fastify Local Service 可在 Mock 模式启动；REST health、内存 session bootstrap、认证后 WebSocket health 事件和 production 同源静态托管已验证。
- Node 24 `node:sqlite`、Drizzle ORM/Kit 1.0 RC、OS 默认数据目录、版本化 migration、WAL、foreign keys、严格文件权限和失败迁移事务回滚已验证；业务 owner schema、播放和真实 Provider 尚未实现。
...
- `packages/contracts/src/` 已包含完整 v1 REST DTO/command、显式 `profileId`、幂等 request、异步 job、事件与错误 Zod schemas；除 S1 health/session/events 外，对应业务实现仍未开始。
- [ADR 0001](docs/adr/0001-toolchain-and-quality.md) 的运行版本、workspace、锁文件、完整根命令族和 Linux GitHub Actions CI 已实装并由真实 run 验证。[ADR 0002](docs/adr/0002-runtime-topology.md) 的 S1 最小拓扑已实装，完整 REST/WS session 安全矩阵仍待 S2-04。
...
- `architecture.md` 中超出 S2-03 平台底座的业务代码结构、API 和运行行为仍是目标设计；当前事实以本节与真实仓库为准。
