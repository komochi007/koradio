...
- S1 工程脚手架阶段门已通过；S2-01～S2-04 的完整 v1 公共 Contracts、SQLite/Drizzle 数据底座、macOS Keychain Secret Store、受控 File Store、结构化脱敏日志和本地 Session/Origin 防护已实现并验证，下一关键任务是 S2-05 DeviceSettings、ProfilePreferences、Health 与数据迁移底座。
...
- `architecture.md` 中超出 S2-04 平台与安全底座的业务代码结构、API 和运行行为仍是目标设计；当前事实以本节与真实仓库为准。
