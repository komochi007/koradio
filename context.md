...
- **UX-31 已完成并验收**：Daily Mix 顶部右侧使用可复用的确定性 SVG 声场组件，72 条极细黑白灰曲线分成三组共享低频结构并交错起伏，辅以三层低透明曲面和四向渐隐；窄窗下画布扩展至 `112%` 并配合分级透明遮罩消除左侧切边，路径在窗口变化时不重算，三组曲线以 10.8～13.2 秒周期缓慢明暗律动，整体以 12 秒周期轻微呼吸，宽幅渐变弧光以 14 秒周期流转，Reduce Motion 下全部停止。`PLAY ALL` 图标改为黑色正圆与白色三角；Programs 区块/列表间距统一为 `24px`/`16px`，Daily 卡片使用手型指针；Radio 进度条悬停或键盘聚焦时以 `180ms` 平滑增高并显示 `10px` 绿色拖动控件。macOS `0.0.242` 已通过包装验证与项目所有者验收。
