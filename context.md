...
- S1 工程脚手架、S2 平台/数据/安全底座和 S3 核心领域与 Provider 后端阶段门已通过；S4-01～S4-04 App Shell、Onboarding、Profile/Settings、服务检测、安全切换、Radio 三态/节目生成交互和唯一 Audio Engine/多标签租约已实现并验证，下一关键任务是 S4-05 Detail Sheet、歌词与串讲跟随。
...
- `architecture.md` 中超出 S3 Profiles/Library/Feedback/Taste/Programs/Playback/生成 Job/Provider Adapters、平台安全底座与 S4-04 Browser Audio Engine 的前端产品功能、native helper 和包装运行行为仍是目标设计；当前事实以本节与真实仓库为准。
