...
- [ADR 0004](docs/adr/0004-provider-feasibility.md) 处于提议状态：Codex、网易云搜索/歌词/歌单和 Apple TTS 已验证；网易云“获取歌曲播放url”端点存在，但当前开放平台应用未授权，S0-06 仍阻塞。
...
- Codex、网易云通过 Backend Adapter 接入；可选 TTS 由 Backend TTS Port 调用 bundled macOS native helper，再使用 Apple `AVSpeechSynthesizer`。网易云真实音乐播放仍需先解除播放 URL 接口授权阻塞。
...
- Provider 可行性尚未完全裁决：网易云搜索、歌词、歌单可用；官方批量播放 URL 端点 `POST /openapi/music/basic/batch/song/playurl/get` 当前返回应用未授权，不能用 `ncm-cli play` 的 `orpheus://` 唤端结果替代 Browser Audio Engine。

## 11. 尚未决定

Agent 不得自行假定：

- Provider 完整可行性裁决、数据库与其他业务依赖的具体包和精确版本。
- 尚未创建的 monorepo、manifest、锁文件、配置、CI workflow 与 script 已经可用。

工具链实现必须遵循 ADR 0001；运行拓扑实现必须遵循 ADR 0002；macOS 包装与交付边界必须遵循 ADR 0003。解除 S0-06 阻塞前，不得把 ADR 0004 当作已接受的 Provider 实现依据。
