...
- Electron 桌面窗口及开发期 standalone PWA 预览使用 `960 × 1600px` 等比完整画布，窄窗口不触发 Mobile 骨架、页面外层不滚动；普通浏览器与手机保留响应式阅读布局。Radio 输入的语音和发送按钮同尺寸，Tab 选中态为白色圆角底配纯黑图标；产品图标使用居中且留白充足的品牌标记。
