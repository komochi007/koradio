...
- S1 工程脚手架和 S2 平台、数据与安全阶段门已通过，S3-01 Profiles、S3-02 Library、S3-03 Feedback/Taste 与 S3-04 Programs/Playback 后端也已完成；幂等 Profiles、Library 搜索/候选池/异步歌单导入、歌词、短期播放解析、七类 append-only Feedback、可重建 TasteProjection、人工 TasteOverrides/EffectiveTaste、Program 原子提交/历史详情、判别式 timeline 和带 lease 的低频 checkpoint 已实现并验证，下一关键任务是 S3-05 Provider Adapters。
...
- `architecture.md` 中超出 S3-04 Profiles/Library/Feedback/Taste/Programs/Playback 与平台安全底座的业务代码结构、API 和运行行为仍是目标设计；当前事实以本节与真实仓库为准。
