...
- S1 工程脚手架和 S2 平台、数据与安全阶段门已通过，S3-01～S3-05 后端任务已完成；Profiles、Library、Feedback/Taste、Programs/Playback，以及 Codex、NetEase、TTS Provider adapters 已实现并验证，下一关键任务是 S3-06 生成 Job、事件排序与恢复 Snapshot。
...
- [ADR 0004](docs/adr/0004-provider-feasibility.md) 已接受：v1 Personal Local Preview 使用 Codex CLI、Backend TypeScript NetEase `linuxapi` Adapter 与 bundled Apple TTS helper；三个 Backend Adapter 已实现，application composition 与 native helper 尚未实现，非官方协议不构成公开分发许可。
- 设计预览可直接打开 `design/assets/prototype/index.html`，但这是零构建设计 fixture，不代表产品可运行。
- `architecture.md` 中超出 S3-05 Profiles/Library/Feedback/Taste/Programs/Playback/Provider Adapters 与平台安全底座的业务代码结构、API 和运行行为仍是目标设计；当前事实以本节与真实仓库为准。
