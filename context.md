...
- S1 工程脚手架和 S2 平台、数据与安全阶段门已通过，S3-01 Profiles 与 S3-02 Library 后端也已完成；幂等 Profiles、Library 搜索/候选池/异步歌单导入、歌词、短期播放解析、默认 TasteOverrides/ProfilePreferences 和切换协调已实现并验证，下一关键任务是 S3-03 Feedback 与 Taste 记忆后端。
...
- Node 24 `node:sqlite`、Drizzle ORM/Kit 1.0 RC、OS 默认数据目录、版本化 migration、WAL、foreign keys、严格文件权限和失败迁移事务回滚已验证；Profile、TasteOverrides、DeviceSettings、ProfilePreferences、MusicTrack、PlaylistSource、LibraryItem 与异步导入 job owner 表已实现，Program、Playback 和真实 Provider 尚未实现。
...
- `architecture.md` 中超出 S3-02 Profiles/Library 与平台安全底座的业务代码结构、API 和运行行为仍是目标设计；当前事实以本节与真实仓库为准。
