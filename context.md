...
- S1 工程脚手架、S2 平台/数据/安全底座、S3 核心领域与 Provider 后端、S4 P0 前端、S5 P1 全量功能与 S6 集成、质量、安全阶段门均已通过；S6-01～S6-05 已依次关闭跨层失败、数据生命周期与恢复、安全/隐私/依赖审计、性能/无障碍回归和内部全质量门。下一关键任务是 S7-01 的受控本机 macOS 包装；真实 Provider 产品组合与 bundled native helper 尚未完成。
...
- `architecture.md` 中超出 S3 Profiles/Library/Feedback/Taste/Programs/Playback/生成 Job/Provider Adapters、平台安全底座、S4 P0 前端体验与 S5 全量功能产品的 native helper、质量加固和包装运行行为仍是目标设计；当前事实以本节与真实仓库为准。
