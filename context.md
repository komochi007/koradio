...
- Desktop standalone PWA 将 `960 × 1600px` 保留为历史高保真原型基线，运行时使用最大 `720px` 宽、`100dvh` 高的中央单列画布且不整体缩放；launcher 默认外窗 `720 × 840px`，低于 `680 × 760px` 时显示窗口过小提示。页面外层不滚动，Queue、DJ、Library 本地音乐、管理内容与 Detail 文本卡片等明确内容层独立滚动。Radio 时间、日期、状态纵向排列，歌曲与来源信息严格单行；Detail 优先使用逐字歌词/TTS marker，缺失时估算逐词时间。
