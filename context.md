...
- S1 工程脚手架和 S2 平台、数据与安全阶段门已通过；S2-01～S2-05 的完整 v1 公共 Contracts、SQLite/Drizzle、DeviceSettings、ProfilePreferences、脱敏 Health、可回滚数据目录迁移、macOS Keychain Secret Store、受控 File Store、结构化脱敏日志和本地 Session/Origin 防护已实现并验证，下一关键任务是 S3-01 Profiles 与 ProfilePreferences 领域闭环。
...
- Node 24 `node:sqlite`、Drizzle ORM/Kit 1.0 RC、OS 默认数据目录、版本化 migration、WAL、foreign keys、严格文件权限和失败迁移事务回滚已验证；DeviceSettings、ProfilePreferences 和数据目录迁移 owner 表已实现，Profiles、Program、播放和真实 Provider 尚未实现。
...
- `packages/contracts/src/` 已包含完整 v1 REST DTO/command、显式 `profileId`、幂等 request、异步 job、事件与错误 Zod schemas；health/session/events、DeviceSettings、ProfilePreferences 和数据目录迁移已有后端实现，其余业务实现仍未开始。
...
- `architecture.md` 中超出 S2-05 平台、配置与安全底座的业务代码结构、API 和运行行为仍是目标设计；当前事实以本节与真实仓库为准。
