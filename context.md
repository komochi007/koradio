...
- S1 工程脚手架和 S2 平台、数据与安全阶段门已通过，S3-01～S3-06 后端任务已完成；Profiles、Library、Feedback/Taste、Programs/Playback、异步生成 Job，以及 Codex、NetEase、TTS Provider adapters 已实现并验证，下一关键任务是 S3-07 Mock Provider 后端闭环。
...
- `architecture.md` 中超出 S3-06 Profiles/Library/Feedback/Taste/Programs/Playback/生成 Job/Provider Adapters 与平台安全底座的业务代码结构、API 和运行行为仍是目标设计；当前事实以本节与真实仓库为准。
