...
- **UX-26 已完成并验收**：指定单曲不以搜索元数据的 `playable` 直接承诺可播。Radio 先筛选原唱录音室原版，再在 15 秒总预算内依次预解析最多 3 条同曲同艺人原版候选；Library 对网络、超时、限流等瞬态解析失败只重试一次并短期缓存成功 URL。候选均不可播时，turn 保留说明和“重新获取 / 换一首”入口，不产生 `PLAY NOW` 卡片，也绝不静默替换为翻唱、纯音乐或其他歌曲。播放 URL 不持久化；安全日志只包含 source、sourceTrackId、稳定失败原因和尝试次数。
...
- **UX-31 已完成并验收**：Daily Mix 顶部右侧使用可复用的确定性 SVG 声场组件，72 条极细黑白灰曲线分成三组共享低频结构并交错起伏，辅以三层低透明曲面和四向渐隐；窄窗下画布扩展至 `112%` 并配合分级透明遮罩消除左侧切边，路径在窗口变化时不重算。三组曲线以 10.8～13.2 秒周期缓慢明暗律动，整体以 12 秒周期只做透明度呼吸；椭圆径向弧光以 9.6 秒匀速扫过三层连续曲面柔光，后半段适度增强，不重复叠加 72 条细线，因此无亚像素抖动与莫尔闪烁，Reduce Motion 下全部停止。`PLAY ALL` 图标改为黑色正圆与白色三角；Programs 区块/列表间距统一为 `24px`/`16px`，Daily 卡片使用手型指针；Radio 进度条悬停或键盘聚焦时以 `180ms` 平滑增高并显示 `10px` 绿色拖动控件。macOS `0.0.249` 已通过包装验证与项目所有者验收。
...
- S1～S6 阶段门以及 S7-01/02/06/07/08/09 均已通过；S7-06 的 Codex/NetEase/Apple TTS 事实只保留为历史验收，当前 TTS 已由 UX-10 替换为 Qwen3-TTS 8-bit。`v1.0.0` 是项目所有者本机正式版；S7-03 签名、公证与外部发布流水线继续后置。
- S7-09 已完成本机桌面入口收敛：固定 `/Applications/Koradio.app` 是唯一 Launchpad 图标，每次正常打开先联网检查可信 `origin/main`；新提交只在独立缓存源码完成 frozen install、构建、strict codesign 与 Electron 包验证后原位替换，失败时不打开旧版。产品由 Electron 主进程加载现有 Web Renderer，不创建第二个 PWA 图标或普通网页标签；更新和服务检查期间先显示本地启动状态页。
...
- Library 通过 MusicProvider Port 接收不可信输出并严格归一化为稳定 source identity；搜索、歌词和短期播放解析使用有界 TTL 缓存，播放解析对瞬态失败只重试一次，播放直链不进入 SQLite，异步歌单导入按完整曲目 ID 清单补齐元数据并在短事务内写入全部可解析歌曲、封面 URL 与可播放状态。
...
- 本机正式版的唯一桌面入口固定为 `/Applications/Koradio.app`；正常启动前从可信 `origin/main` 本机构建更新并 fail-closed，不修改开发工作树、不创建第二个 PWA 图标或普通网页标签。
- 当前 `v1.0.0` 只支持项目所有者从可信源码在受控本机使用；公开下载与外部分发后置且需要单独授权，届时 Developer ID、Apple 公证、Gatekeeper 与独立干净环境验收仍是硬门。
