...
- S1 工程脚手架和 S2 平台、数据与安全阶段门已通过，S3-01 Profiles、S3-02 Library 与 S3-03 Feedback/Taste 后端也已完成；幂等 Profiles、Library 搜索/候选池/异步歌单导入、歌词、短期播放解析、七类 append-only Feedback、可重建 TasteProjection、人工 TasteOverrides/EffectiveTaste 和切换协调已实现并验证，下一关键任务是 S3-04 Programs 与 Playback 领域模型。
...
- React/Vite App Shell 与 Fastify Local Service 可在 Mock 模式启动；短期进程内 session bootstrap、认证 REST health、认证后 WebSocket health 事件、401 后重新 bootstrap 和 production 同源静态托管已验证。
- Node 24 `node:sqlite`、Drizzle ORM/Kit 1.0 RC、OS 默认数据目录、版本化 migration、WAL、foreign keys、严格文件权限和失败迁移事务回滚已验证；Profile、TasteProjection、TasteOverrides、FeedbackEvent、DeviceSettings、ProfilePreferences、MusicTrack、PlaylistSource、LibraryItem 与异步导入 job owner 表已实现，Program、Playback 和真实 Provider 尚未实现。
...
- `packages/contracts/src/` 已包含完整 v1 REST DTO/command、显式 `profileId`、幂等 request、异步 job、事件与错误 Zod schemas；health/session/events、Profiles、Library、DeviceSettings、ProfilePreferences 和数据目录迁移已有后端实现。
...
- [ADR 0004](docs/adr/0004-provider-feasibility.md) 已接受：v1 Personal Local Preview 使用 Codex CLI、Backend TypeScript NetEase `linuxapi` Adapter 与 bundled Apple TTS helper；真实 Adapter 尚未实现，非官方协议不构成公开分发许可。
- 设计预览可直接打开 `design/assets/prototype/index.html`，但这是零构建设计 fixture，不代表产品可运行。
- `architecture.md` 中超出 S3-03 Profiles/Library/Feedback/Taste 与平台安全底座的业务代码结构、API 和运行行为仍是目标设计；当前事实以本节与真实仓库为准。
