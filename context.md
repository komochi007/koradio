...
- 项目处于 S1 工程脚手架阶段；S1-01 已完成运行版本与 workspace 基础。
...
- 根 manifest、pnpm workspace、单一锁文件、Node 版本文件、四个目标边界 manifest 与基础 TypeScript project references 已创建；frozen install 和最小 typecheck 可运行。
- 产品仍无源码入口、开发服务、测试配置和构建脚本，当前不可启动、测试或构建 Koradio 产品。
- [ADR 0001](docs/adr/0001-toolchain-and-quality.md) 已选定工具链、质量工具、命令族与 CI 基线；其中 S1-01 运行版本、workspace、锁文件和最小 typecheck 已实装，其余质量命令待 S1-02/S1-04。[ADR 0002](docs/adr/0002-runtime-topology.md) 已选定运行拓扑、端口、Origin allowlist 与本地 session bootstrap，仍待 S1/S2 实装。
...
- 数据库与其他业务依赖的具体包和精确版本。
- 尚未创建的产品源码、质量配置、CI workflow、`dev`、测试与构建 script 已经可用。
